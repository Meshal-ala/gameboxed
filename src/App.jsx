...
const PLATFORMS_MAP = {
  4: { name: "PC", icon: "🖥️", color: "#00d4ff" },
  187: { name: "PS5", icon: "🎮", color: "#006FCD" },
  18: { name: "PS4", icon: "🎮", color: "#006FCD" },
  1: { name: "Xbox One", icon: "🟢", color: "#107C10" },
  186: { name: "Xbox X|S", icon: "🟢", color: "#107C10" },
  7: { name: "Switch", icon: "🔴", color: "#E60012" },
};
...
const loadUserData = () => {
  try { return JSON.parse(localStorage.getItem("gb_user") || "{}"); } catch { return {}; }
};
const saveUserData = (data) => { localStorage.setItem("gb_user", JSON.stringify(data)); };

/* ── API helpers ── */
const fetchGames = async (params = "") => {
  try {
    const r = await fetch(`${API}/games?key=${API_KEY}&page_size=12${params}`);
    const d = await r.json(); return d.results || [];
  } catch { return []; }
};
const fetchGameDetail = async (id) => {
  try {
    const r = await fetch(`${API}/games/${id}?key=${API_KEY}`);
    return await r.json();
  } catch { return null; }
};
const searchGamesAPI = async (query) => {
  try {
    const r = await fetch(`${API}/games?key=${API_KEY}&search=${encodeURIComponent(query)}&page_size=20`);
    const d = await r.json(); return d.results || [];
  } catch { return []; }
};
...
const normalizeGame = (g) => ({
  ...
  reviews: g.ratings_count || 0,
  genre: g.genres?.map(x => x.name).join(", ") || "Unknown",
  platforms: (g.parent_platforms || []).map(p => {
    const m = PLATFORMS_MAP[p.platform.id];
    return m ? m.name : null;
  }).filter(Boolean).slice(0, 4),
  platformColors: (g.parent_platforms || []).map(p => PLATFORMS_MAP[p.platform.id]).filter(Boolean),
...
const Loader = () => <div style={{ display: "flex", justifyContent: "center", padding: 40 }}>
  <div style={{ width: 32, height: 32, border: "3px solid #1a1a2e", borderTopColor: "#00d4ff", borderRadius: "50%",
...
const GameCard = ({ game, onClick, wide, delay = 0, mobile, userData }) => {
  ...
  return <div onClick={() => onClick?.(game)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
    ...
    {game.metacritic && <div style={{ position: "absolute", top: 8, right: 8, width: 32, height: 32, borderRadius: 8,
      ...
      color: game.metacritic >= 75 ? "#00ff88" : game.metacritic >= 50 ? "#FFD700" : "#ff4444" }}>
      {game.metacritic}</div>}
    ...
    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: wide ? "16px 18px" : mobile ? "10px 12px" : "12px 14px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3, fontSize: 10, color: "rgba(255,255,255,.55)" }}>
        <span>{game.year}</span><span style={{ opacity: .3 }}>·</span><span>{game.genre?.split(",")[0]}</span>
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }}>
        ...
        <div style={{ display: "flex", gap: 3 }}>
          {(game.platformColors || []).slice(0, 3).map((p, i) => <span key={i} style={{ fontSize: 8, fontWeight: 800, padding: "2px 5px", borderRadius: 3, background: "rgba(255,255,255,.1)", color: p.color }}>{p.name}</span>)}
        </div>
      </div>
    </div>
  </div>;
...
const Modal = ({ game, onClose, mobile, userData, setUserData }) => {
  ...
  useEffect(() => {
    setLoading(true);
    fetchGameDetail(game.id).then(d => { setDetail(d); setLoading(false); });
  }, [game.id]);

  const updateUser = (field, val) => {
    const newData = { ...userData, [game.id]: { ...ud, [field]: val, title: game.title, img: game.img } };
    ...
    <div onClick={e => e.stopPropagation()} style={{ background: "#0c0c18", width: "100%", maxWidth: mobile ? "100%" : 700,
      ...
      <div style={{ position: "relative", height: mobile ? 180 : 220, overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", bottom: 14, left: mobile ? 16 : 24, right: mobile ? 16 : 24, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div><h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 900, margin: 0, textShadow: "0 4px 16px rgba(0,0,0,.8)" }}>{game.title}</h2>
            <div style={{ color: "rgba(255,255,255,.45)", fontSize: 12, marginTop: 2 }}>{game.year} · {game.genre?.split(",")[0]}{game.reviews > 0 && " · " + (game.reviews/1000).toFixed(1) + "k reviews"}</div></div>
      ...
      <div style={{ padding: mobile ? "14px 16px 28px" : "18px 24px 28px" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
          {(game.platformColors || []).map((p, i) => <span key={i} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700,
          ...
          {game.metacritic && <span style={{ padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700,
            background: game.metacritic >= 75 ? "#00ff8810" : "#FFD70010",
            color: game.metacritic >= 75 ? "#00ff88" : "#FFD700",
            border: "1px solid " + (game.metacritic >= 75 ? "#00ff88" : "#FFD700") + "28" }}>
            Metacritic: {game.metacritic}</span>}
        </div>

        {/* Description */}
        {loading ? <Loader /> : detail?.description_raw && (
          <p style={{ color: "#777", fontSize: 13, lineHeight: 1.7, marginBottom: 18, maxHeight: 80, overflow: "hidden",
            WebkitMaskImage: "linear-gradient(to bottom, black 60%, transparent)" }}>
            {detail.description_raw.slice(0, 300)}...</p>
        )}
        ...
        <div style={{ padding: mobile ? 14 : 18, borderRadius: 14, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.05)", marginBottom: 18 }}>
          ...
          <Stars rating={mr} size={mobile ? 24 : 26} interactive onRate={v => { setMr(v); updateUser("myRating", v); }} />
          <div style={{ fontSize: 10, color: "#555", fontWeight: 700, letterSpacing: ".08em", marginTop: 14, marginBottom: 6 }}>STATUS</div>
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
            {Object.entries(STATUS_CFG).map(([k, c]) => <button key={k} onClick={() => { setSt(k); updateUser("status", k); }} style={{
        ...
        {game.screenshots?.length > 1 && <>
          ...
          <div className="hide-scroll" style={{ display: "flex", gap: 8, overflowX: "auto", marginBottom: 18, paddingBottom: 4 }}>
            {game.screenshots.slice(1, 6).map((s, i) => <img key={i} src={s} alt="" loading="lazy"
              style={{ height: 80, borderRadius: 10, objectFit: "cover", flexShrink: 0 }} />)}
          </div>
        </>}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          <div style={{ padding: 12, borderRadius: 12, background: "rgba(255,255,255,.025)", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 900, color: "#FFD700", marginTop: 4 }}>{game.rating || "—"}<span style={{ fontSize: 11, color: "#444" }}>/5</span></div>
          </div>
          <div style={{ padding: 12, borderRadius: 12, background: "rgba(255,255,255,.025)", textAlign: "center" }}>
            <div style={{ fontSize: 10, color: "#444", fontWeight: 700 }}>METACRITIC</div>
            <div style={{ fontSize: 20, fontWeight: 900, color: game.metacritic >= 75 ? "#00ff88" : "#FFD700", marginTop: 4 }}>{game.metacritic || "—"}</div>
          </div>
          <div style={{ padding: 12, borderRadius: 12, background: "rgba(255,255,255,.025)", textAlign: "center" }}>
            <div style={{ fontSize: 10, color: "#444", fontWeight: 700 }}>MY RATING</div>
            <div style={{ fontSize: 20, fontWeight: 900, color: "#00d4ff", marginTop: 4 }}>{mr || "—"}<span style={{ fontSize: 11, color: "#444" }}>/5</span></div>
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetchGames("&ordering=-added&dates=2023-01-01,2025-12-31"),
      fetchGames("&ordering=-rating&metacritic=80,100"),
      fetchGames("&ordering=-released&dates=2024-06-01,2025-12-31"),
    ]).then(([t, r, n]) => {
      setTrending(t.map(normalizeGame));
      setTopRated(r.map(normalizeGame));
      setNewReleases(n.map(normalizeGame));
      setLoading(false);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(() => {
      searchGamesAPI(search).then(r => { setSearchResults(r.map(normalizeGame)); setSearching(false); });
    }, 400);
  }, [search]);

  // Library: user's saved games
  const libraryGames = Object.entries(userData).filter(([_, v]) => v.status).map(([id, v]) => {
    const found = [...trending, ...topRated, ...newReleases, ...searchResults].find(g => g.id === parseInt(id));
    return found || { id: parseInt(id), title: v.title || "Unknown", img: v.img || "", year: "", genre: "", rating: null, platforms: [], platformColors: [] };
  });
  ...
  return <div style={{ fontFamily: "'DM Sans','Outfit',system-ui,sans-serif", background: "#08080f", color: "#fff", minHeight: "100vh" }}>
    ...
    <main style={{ maxWidth: 1200, margin: "0 auto", padding: mobile ? "12px 14px 100px" : "20px 18px 80px" }}>
      ...
      {page === "search" && <div style={{ animation: "fadeIn .4s" }}>
        <h2 style={{ fontFamily: "'Outfit'", fontSize: mobile ? 22 : 26, fontWeight: 900, marginBottom: 18 }}>
          {search ? `Results for "${search}"` : "Search for any game..."}</h2>
        {searching ? <Loader /> :
          searchResults.length > 0 ? <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(auto-fill,minmax(220px,1fr))", gap: mobile ? 10 : 14 }}>
            {searchResults.map((g, i) => <GameCard key={g.id} game={g} delay={i * 40} onClick={setSel} mobile={mobile} userData={userData} />)}
          </div> : search && <div style={{ textAlign: "center", padding: 60, color: "#444" }}>
      ...
      {page === "home" && <div style={{ animation: "fadeIn .4s" }}>
        <div style={{ borderRadius: mobile ? 18 : 22, padding: mobile ? "28px 22px" : "44px 36px", marginBottom: mobile ? 24 : 36,
          ...
          <p style={{ color: "#666", fontSize: mobile ? 13 : 14, maxWidth: 460, lineHeight: 1.7, marginBottom: mobile ? 18 : 24, position: "relative" }}>
            Discover 500,000+ games. Rate, review, and build your ultimate collection.</p>
        ...
        {loading ? <Loader /> : <>
          <Section title="🔥 Trending" mobile={mobile}>
            <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
              {trending.slice(0, 8).map((g, i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i * 60} onClick={setSel} mobile={mobile} userData={userData} /></div>)}
            </div>
          </Section>

          <Section title="⭐ Top Rated" action="See All →" onAction={() => setPage("explore")} mobile={mobile}>
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: mobile ? 10 : 14 }}>
              {topRated.slice(0, 2).map((g, i) => <GameCard key={g.id} game={g} wide delay={i * 100} onClick={setSel} mobile={mobile} userData={userData} />)}
            </div>
          </Section>

          <Section title="🆕 New Releases" mobile={mobile}>
            <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
              {newReleases.slice(0, 8).map((g, i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i * 60} onClick={setSel} mobile={mobile} userData={userData} /></div>)}
            </div>
          </Section>
        </>}
      </div>}
          ...
          <div className="hide-scroll" style={{ display: "flex", gap: 5, marginBottom: 18, overflowX: "auto", paddingBottom: 8 }}>
            {[["all","All"],["playing","Playing"],["completed","Done"],["backlog","Backlog"],["wishlist","Wishlist"],["dropped","Dropped"]].map(([k,l]) =>
              <button key={k} onClick={() => setLibFilter(k)} style={{ padding: "5px 13px", borderRadius: 18, fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap",
                ...
                color: libFilter === k ? "#00d4ff" : "#555" }}>{l} <span style={{ opacity: .5 }}>{k === "all" ? libraryGames.length : libraryGames.filter(g => userData[g.id]?.status === k).length}</span></button>)}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(auto-fill,minmax(220px,1fr))", gap: mobile ? 10 : 14 }}>
            {filteredLib.map((g, i) => <GameCard key={g.id} game={g} delay={i * 50} onClick={setSel} mobile={mobile} userData={userData} />)}
          </div>
        </> : <div style={{ textAlign: "center", padding: 60, color: "#555" }}>
          <div style={{ fontSize: 56, marginBottom: 12 }}>🎮</div>
          <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>Your library is empty</div>
          <div style={{ fontSize: 13, color: "#444", marginBottom: 20 }}>Search for games and add them to start building your collection!</div>
          <button onClick={() => { setPage("search"); setSearchOpen(true); }} style={{ padding: "10px 24px", borderRadius: 10, border: "none",
            background: "linear-gradient(135deg,#00d4ff,#0088bb)", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Search Games</button>
        </div>}
      </div>}
      ...
      {page === "explore" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        {loading ? <Loader /> : <>
          <Section title="⭐ Highest Rated" mobile={mobile}>
            <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
              {topRated.map((g, i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i * 60} onClick={setSel} mobile={mobile} userData={userData} /></div>)}
            </div>
          </Section>
          <Section title="🔥 Most Popular" mobile={mobile}>
            <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
              {trending.map((g, i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i * 60} onClick={setSel} mobile={mobile} userData={userData} /></div>)}
            </div>
          </Section>
          <Section title="🆕 Latest" mobile={mobile}>
            <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
              {newReleases.map((g, i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i * 60} onClick={setSel} mobile={mobile} userData={userData} /></div>)}
            </div>
          </Section>
        </>}
      </div>}
      ...
      {page === "stats" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        {libraryGames.length > 0 ? (() => {
          const byStatus = (s) => libraryGames.filter(g => userData[g.id]?.status === s).length;
          ...
          return <>
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(auto-fit,minmax(140px,1fr))", gap: 10, marginBottom: 28 }}>
              {[{ l: "In Library", v: libraryGames.length, i: "🎮", c: "#00d4ff" }, { l: "Completed", v: byStatus("completed"), i: "✅", c: "#00ff88" },
                { l: "Playing", v: byStatus("playing"), i: "▶️", c: "#8b5cf6" }, { l: "Avg Rating", v: avgR + "★", i: "⭐", c: "#FFD700" }].map((s, i) =>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 28 }}>
              {Object.entries(STATUS_CFG).map(([k, c]) => {
                const cnt = byStatus(k), max = libraryGames.length || 1;
                return <div key={k} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1, height: 24, background: "rgba(255,255,255,.03)", borderRadius: 6, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: cnt > 0 ? (cnt / max * 100) + "%" : "0%", background: c.color, borderRadius: 6, display: "flex", alignItems: "center", paddingLeft: 8, opacity: .8, transition: "width 1s" }}>
                      {cnt > 0 && <span style={{ fontSize: 10, fontWeight: 800 }}>{cnt}</span>}</div></div></div>; })}
            </div>
          </>;
        })() : <div style={{ textAlign: "center", padding: 60, color: "#555" }}>
          <div style={{ fontSize: 48, marginBottom: 10 }}>📊</div>
          <div style={{ fontSize: 15, fontWeight: 700 }}>No stats yet</div>
          <div style={{ fontSize: 13, color: "#444", marginTop: 4 }}>Add games to your library to see your stats!</div>
        </div>}
      </div>}
    </main>
