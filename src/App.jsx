...
const SC={completed:{c:"#00ff88",l:"Completed",e:"✅"},playing:{c:"#00d4ff",l:"Playing",e:"🎮"},wishlist:{c:"#FFD700",l:"Wishlist",e:"⭐"},dropped:{c:"#ff4444",l:"Dropped",e:"❌"},backlog:{c:"#8b5cf6",l:"Backlog",e:"📚"}};
...
const createList=async(u,t,d)=>await supabase.from("lists").insert({user_id:u,title:t,description:d}).select().single();
...
const loadFeed=async uid=>{
  ...
  const{data}=await supabase.from("activities").select("*,profiles(display_name,username)").in("user_id",ids).order("created_at",{ascending:false}).limit(30);
  return data||[]};
const loadAllFeed=async()=>{const{data}=await supabase.from("activities").select("*,profiles(display_name,username)").order("created_at",{ascending:false}).limit(30);return data||[]};

/* Reviews */
const loadGameReviews=async gid=>{const{data}=await supabase.from("reviews").select("*,profiles(display_name,username)").eq("game_id",gid).order("created_at",{ascending:false}).limit(20);return data||[]};
...
const loadRecentReviews=async()=>{const{data}=await supabase.from("reviews").select("*,profiles(display_name,username)").order("created_at",{ascending:false}).limit(10);return data||[]};
...
const isFollowing=async(me,them)=>{const{data}=await supabase.from("follows").select("id").eq("follower_id",me).eq("following_id",them).single();return!!data};
const getFollowCounts=async uid=>{const{count:ers}=await supabase.from("follows").select("*",{count:"exact",head:true}).eq("following_id",uid);const{count:ing}=await supabase.from("follows").select("*",{count:"exact",head:true}).eq("follower_id",uid);return{followers:ers||0,following:ing||0}};
...
const Stars=({rating=0,size=14,interactive,onRate})=>{const[hov,setHov]=useState(0);const ac=hov||rating;
  ...
  return<div style={{display:"flex",gap:1,alignItems:"center"}}>{[1,2,3,4,5].map(s=>{const f=s<=Math.floor(ac),h=!f&&s-.5<=ac&&s>ac-.5;
    return<span key={s} onClick={e=>clk(s,e)} onMouseEnter={()=>interactive&&setHov(s)} onMouseLeave={()=>interactive&&setHov(0)}
      style={{fontSize:size,cursor:interactive?"pointer":"default",position:"relative",lineHeight:1,color:f?"#FFD700":h?"#FFD700":"#2a2a3a",transition:"all .15s",transform:interactive&&s<=Math.ceil(hov)?"scale(1.15)":"scale(1)",filter:(f||h)?"drop-shadow(0 0 3px rgba(255,215,0,.4))":"none"}}>
      {h?<><span style={{position:"absolute",overflow:"hidden",width:"50%"}}>★</span><span style={{color:"#2a2a3a"}}>★</span></>:"★"}</span>})}
    {interactive&&ac>0&&<span style={{fontSize:size*.65,color:"#FFD700",fontWeight:700,marginLeft:3}}>{ac}</span>}</div>};
...
const GC=({game:g,onClick,delay=0,mobile:m,userData:ud,size="normal"})=>{const[hov,setHov]=useState(false);const[vis,setVis]=useState(false);const[err,setErr]=useState(false);
  ...
  return<div onClick={()=>onClick?.(g)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    style={{borderRadius:tn?6:10,overflow:"hidden",cursor:"pointer",position:"relative",aspectRatio:tn?"3/4":"2/3",opacity:vis?1:0,transform:vis?(hov&&!m?"translateY(-3px)":"none"):"translateY(10px)",transition:"all .3s cubic-bezier(.22,1,.36,1)",boxShadow:hov&&!m?"0 12px 30px rgba(0,0,0,.5)":"0 2px 8px rgba(0,0,0,.15)"}}>
    {!err&&g.img?<img src={g.img} alt={g.title} onError={()=>setErr(true)} loading="lazy" style={{width:"100%",height:"100%",objectFit:"cover",transition:"transform .4s",transform:hov&&!m?"scale(1.04)":"scale(1)"}}/>
      :<div style={{width:"100%",height:"100%",background:"linear-gradient(135deg,#16213e,#0f3460)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:tn?16:32,color:"#ffffff15"}}>🎮</div>}
    <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,.88) 0%,rgba(0,0,0,.05) 55%,transparent 100%)"}}/>
    {u?.status&&!tn&&<div style={{position:"absolute",top:5,left:5,padding:"2px 6px",borderRadius:8,background:"rgba(0,0,0,.6)",fontSize:7,fontWeight:800,color:SC[u.status]?.c}}>{SC[u.status]?.l.toUpperCase()}</div>}
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:tn?"4px":"8px 9px"}}>
      <div style={{fontSize:tn?8:12,fontWeight:700,lineHeight:1.2,overflow:"hidden",textOverflow:"ellipsis",display:"-webkit-box",WebkitLineClamp:tn?1:2,WebkitBoxOrient:"vertical"}}>{g.title}</div>
      {!tn&&g.rating&&<div style={{display:"flex",alignItems:"center",gap:2,marginTop:2}}><Stars rating={Math.round(g.rating)} size={7}/><span style={{fontSize:8,color:"#FFD700",fontWeight:700}}>{g.rating}</span></div>}
      {!tn&&<div style={{fontSize:7,color:"rgba(255,255,255,.3)",marginTop:1}}>{g.year}</div>}
    </div></div>};

const WC=({game:g,onClick,mobile:m,userData:ud,label})=>{const[hov,setHov]=useState(false);
  return<div onClick={()=>onClick?.(g)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    style={{borderRadius:12,overflow:"hidden",cursor:"pointer",position:"relative",aspectRatio:m?"16/9":"21/9",boxShadow:hov&&!m?"0 16px 40px rgba(0,0,0,.5)":"0 2px 12px rgba(0,0,0,.2)",transition:"all .3s",marginBottom:m?20:28}}>
    ...
    <div style={{position:"absolute",bottom:m?14:20,left:m?14:24}}>{label&&<div style={{fontSize:9,color:"#00d4ff",fontWeight:700,letterSpacing:".1em",marginBottom:3}}>{label}</div>}
      <h3 style={{fontSize:m?18:28,fontWeight:900,margin:0,lineHeight:1.15}}>{g.title}</h3>
      <div style={{fontSize:m?10:12,color:"rgba(255,255,255,.4)",marginTop:3}}>{g.year} · {g.genre}</div></div></div>};

const Sec=({title,children,action,onAction})=><div style={{marginBottom:28}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
  <h3 style={{fontSize:13,fontWeight:700,color:"#888",letterSpacing:".04em"}}>{title}</h3>{action&&<span onClick={onAction} style={{fontSize:11,color:"#444",cursor:"pointer",fontWeight:600}}>More →</span>}</div>{children}</div>;
...
const EP=({profile:p,onClose,onSave})=>{const[n,setN]=useState(p?.display_name||"");const[u,setU]=useState(p?.username||"");const[b,setB]=useState(p?.bio||"");const[ld,setLd]=useState(false);
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,.85)",backdropFilter:"blur(24px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#111118",borderRadius:20,width:"100%",maxWidth:380,padding:"28px 24px",border:"1px solid rgba(255,255,255,.06)"}}>
      ...
      <label style={{fontSize:11,color:"#555",fontWeight:700,letterSpacing:".06em",display:"block",marginBottom:4}}>USERNAME</label><input value={u} onChange={e=>setU(e.target.value)} style={i}/>
      <label style={{fontSize:11,color:"#555",fontWeight:700,letterSpacing:".06em",display:"block",marginBottom:4}}>BIO</label><textarea value={b} onChange={e=>setB(e.target.value)} rows={3} style={{...i,resize:"none",fontFamily:"inherit"}}/>
      <div style={{display:"flex",gap:8,marginTop:4}}><button onClick={onClose} style={{flex:1,padding:"11px",borderRadius:10,border:"1px solid rgba(255,255,255,.08)",background:"transparent",color:"#888",fontSize:13,fontWeight:700,cursor:"pointer"}}>Cancel</button>
...
const GD=({game:g,onClose,mobile:m,userData:ud,setUserData,user,setShowAuth,refreshFeed})=>{
  ...
  const[revs,setRevs]=useState([]);const[revText,setRevText]=useState("");const[revRating,setRevRating]=useState(0);const[posting,setPosting]=useState(false);
  ...
  const submitReview=async()=>{if(!user||!revText.trim())return;setPosting(true);await postReview(user.id,g,revRating,revText);setRevText("");setRevRating(0);const r=await loadGameReviews(g.id);setRevs(r);setPosting(false);refreshFeed?.()};
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.92)",backdropFilter:"blur(20px)",display:"flex",alignItems:m?"flex-end":"center",justifyContent:"center",animation:"fadeIn .2s",padding:m?0:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#111118",width:"100%",maxWidth:m?"100%":640,maxHeight:m?"92vh":"85vh",borderRadius:m?"20px 20px 0 0":20,overflow:"auto",border:m?"none":"1px solid rgba(255,255,255,.06)",animation:m?"slideFromBottom .3s cubic-bezier(.22,1,.36,1)":"slideUp .3s cubic-bezier(.22,1,.36,1)"}}>
      ...
      <div style={{position:"relative",height:m?160:200,overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",bottom:12,left:m?16:20,right:m?16:20}}><h2 style={{fontSize:m?20:26,fontWeight:900,margin:0}}>{g.title}</h2>
      ...
      <div style={{padding:m?"12px 16px 28px":"16px 20px 24px"}}>
        ...
        <div style={{marginBottom:16}}><div style={{fontSize:10,color:"#444",fontWeight:700,marginBottom:6,letterSpacing:".06em"}}>YOUR RATING</div><Stars rating={mr} size={m?24:28} interactive onRate={v=>sv("myRating",v)}/></div>
        <div style={{marginBottom:16}}><div style={{fontSize:10,color:"#444",fontWeight:700,marginBottom:6,letterSpacing:".06em"}}>STATUS</div>
        ...
        <div style={{display:"flex",gap:0,borderBottom:"1px solid rgba(255,255,255,.04)",marginBottom:14}}>
          {["about","reviews","screenshots"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"8px 14px",background:"none",border:"none",fontSize:12,fontWeight:700,cursor:"pointer",color:tab===t?"#fff":"#444",borderBottom:tab===t?"2px solid #00d4ff":"2px solid transparent",textTransform:"capitalize"}}>{t}{t==="reviews"&&revs.length>0&&<span style={{marginLeft:3,fontSize:9,color:"#555"}}>{revs.length}</span>}</button>)}</div>

        {tab==="about"&&(ldg?<Loader/>:det?.description_raw&&<p style={{color:"#777",fontSize:13,lineHeight:1.7,maxHeight:160,overflow:"hidden",WebkitMaskImage:"linear-gradient(to bottom,black 70%,transparent)"}}>{det.description_raw.slice(0,600)}</p>)}

        {tab==="reviews"&&<div>
          {/* Write review */}
          {user&&<div style={{marginBottom:16,padding:14,borderRadius:12,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)"}}>
            <div style={{fontSize:10,color:"#555",fontWeight:700,marginBottom:6}}>WRITE A REVIEW</div>
            <Stars rating={revRating} size={18} interactive onRate={setRevRating}/>
            <textarea value={revText} onChange={e=>setRevText(e.target.value)} rows={3} placeholder="What did you think?" style={{width:"100%",marginTop:8,padding:"10px 12px",borderRadius:8,border:"1px solid rgba(255,255,255,.06)",background:"rgba(255,255,255,.03)",color:"#fff",fontSize:13,outline:"none",resize:"none",fontFamily:"inherit"}}/>
            <button onClick={submitReview} disabled={posting||!revText.trim()} style={{marginTop:8,padding:"8px 16px",borderRadius:8,border:"none",background:revText.trim()?"#00d4ff":"#222",color:revText.trim()?"#000":"#555",fontSize:12,fontWeight:800,cursor:revText.trim()?"pointer":"default"}}>{posting?"Posting...":"Post Review"}</button>
          </div>}
          {revs.length>0?revs.map((r,i)=><div key={r.id} style={{padding:12,borderRadius:10,background:"rgba(255,255,255,.02)",marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              <div style={{width:28,height:28,borderRadius:14,background:"linear-gradient(135deg,#00d4ff,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800}}>{(r.profiles?.display_name||"?").charAt(0).toUpperCase()}</div>
            ...
            <p style={{color:"#999",fontSize:12.5,lineHeight:1.6,margin:0}}>{r.text}</p></div>)
          :<div style={{textAlign:"center",padding:30,color:"#444",fontSize:12}}>No reviews yet{user?" — be the first!":""}</div>}
        </div>}
...
export default function App(){
  ...
  const[ud,setUd]=useState({});const[user,setUser]=useState(null);const[prof,setProf]=useState(null);const[fCounts,setFC]=useState({followers:0,following:0});
  ...
  useEffect(()=>{supabase.auth.getSession().then(({data:{session}})=>{const u=session?.user||null;setUser(u);if(u){lcl(u.id).then(setUd);lp(u.id).then(setProf);loadLists(u.id).then(setMyLists);loadFeed(u.id).then(setFeed);getFollowCounts(u.id).then(setFC)}else{loadAllFeed().then(setFeed)}});
    loadRecentReviews().then(setRecentRevs);
    const{data:{subscription}}=supabase.auth.onAuthStateChange((_,session)=>{const u=session?.user||null;setUser(u);if(u){lcl(u.id).then(setUd);lp(u.id).then(setProf);loadLists(u.id).then(setMyLists);loadFeed(u.id).then(setFeed);getFollowCounts(u.id).then(setFC)}else{setUd({});setProf(null);setMyLists([]);loadAllFeed().then(setFeed)}});return()=>subscription.unsubscribe()},[]);
  ...
  useEffect(()=>{if(stt.current)clearTimeout(stt.current);if(!q.trim()){setSr([]);setPeopleSr([]);setListsSr([]);return}setSng(true);
    stt.current=setTimeout(async()=>{const[games,people,lists]=await Promise.all([sga(q),searchPeople(q),searchLists(q)]);setSr(games.map(nm));setPeopleSr(people);setListsSr(lists);setSng(false)},400)},[q]);
  ...
  return<div style={{fontFamily:"'DM Sans','Outfit',system-ui,sans-serif",background:"#0b0b10",color:"#fff",minHeight:"100vh"}}>
    ...
    <main style={{maxWidth:1100,margin:"0 auto",padding:m?"8px 12px 86px":"16px 24px 50px"}}>
      ...
      {pg==="search"&&<div style={{animation:"fadeIn .3s",paddingTop:10}}>
        ...
        <div style={{display:"flex",gap:0,borderBottom:"1px solid rgba(255,255,255,.04)",marginBottom:16}}>
          {[{k:"games",l:"Games",c:sr.length},{k:"people",l:"People",c:peopleSr.length},{k:"lists",l:"Lists",c:listsSr.length}].map(t=>
            <button key={t.k} onClick={()=>setSTab(t.k)} style={{padding:"8px 16px",background:"none",border:"none",fontSize:12,fontWeight:700,cursor:"pointer",color:sTab===t.k?"#fff":"#444",borderBottom:sTab===t.k?"2px solid #00d4ff":"2px solid transparent"}}>{t.l}{q&&!sng&&<span style={{marginLeft:4,fontSize:10,color:"#555"}}>{t.c}</span>}</button>)}</div>
        {sng?<Loader/>:<>
          {sTab==="games"&&(sr.length>0?<div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(auto-fill,minmax(140px,1fr))",gap:m?6:10}}>{sr.map((g,i)=><GC key={g.id} game={g} delay={i*25} onClick={setSel} mobile={m} userData={ud}/>)}</div>:q&&<div style={{textAlign:"center",padding:50,color:"#444"}}>No games</div>)}
          {sTab==="people"&&(peopleSr.length>0?<div style={{display:"flex",flexDirection:"column",gap:8}}>{peopleSr.map(p=><div key={p.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:12,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)"}}>
            <div style={{width:40,height:40,borderRadius:20,background:"linear-gradient(135deg,#00d4ff,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:800}}>{(p.display_name||"?").charAt(0).toUpperCase()}</div>
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{p.display_name}</div>{p.username&&<div style={{fontSize:11,color:"#555"}}>@{p.username}</div>}</div>
          </div>)}</div>:q&&<div style={{textAlign:"center",padding:50,color:"#444"}}>No people</div>)}
          {sTab==="lists"&&(listsSr.length>0?<div style={{display:"flex",flexDirection:"column",gap:8}}>{listsSr.map(l=><div key={l.id} style={{padding:"12px 16px",borderRadius:12,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)"}}>
            <div style={{fontSize:14,fontWeight:700}}>📝 {l.title}</div><div style={{fontSize:10,color:"#555",marginTop:2}}>{l.game_ids?.length||0} games · by {l.profiles?.display_name||"Unknown"}</div></div>)}</div>:q&&<div style={{textAlign:"center",padding:50,color:"#444"}}>No lists</div>)}
        </>}</div>}
      ...
      {pg==="home"&&<div style={{animation:"fadeIn .4s"}}>
        ...
        {ld?<Loader/>:<>
          ...
          <Sec title="POPULAR THIS WEEK" action="More →" onAction={()=>setPg("explore")}>
            <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>{popular.slice(1,7).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>

          {/* Recent Reviews from community */}
          {recentRevs.length>0&&<Sec title="RECENT REVIEWS">
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {recentRevs.slice(0,4).map(r=><div key={r.id} style={{display:"flex",gap:10,padding:"10px 12px",borderRadius:10,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)"}}>
                {r.game_img&&<img src={r.game_img} alt="" style={{width:44,height:60,borderRadius:6,objectFit:"cover",flexShrink:0}}/>}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                    <div style={{width:20,height:20,borderRadius:10,background:"linear-gradient(135deg,#00d4ff,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:800}}>{(r.profiles?.display_name||"?").charAt(0).toUpperCase()}</div>
                    <span style={{fontSize:11,fontWeight:700}}>{r.profiles?.display_name||"User"}</span>
                    <span style={{fontSize:10,color:"#444"}}>reviewed</span>
                    <span style={{fontSize:11,fontWeight:700,color:"#00d4ff"}}>{r.game_title}</span>
                    <span style={{fontSize:9,color:"#333",marginLeft:"auto"}}>{timeAgo(r.created_at)}</span></div>
                  {r.rating>0&&<Stars rating={r.rating} size={8}/>}
                  <p style={{color:"#777",fontSize:11,lineHeight:1.5,margin:"3px 0 0",overflow:"hidden",textOverflow:"ellipsis",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{r.text}</p>
                </div></div>)}</div></Sec>}

          <Sec title="JUST RELEASED"><div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>{fresh.slice(0,6).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>

          {best[0]&&<WC game={best[0]} onClick={setSel} mobile={m} userData={ud} label="HIGHEST RATED"/>}

          <Sec title="ALL-TIME GREATS"><div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>{best.slice(1,7).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>

          {soon.length>0&&<Sec title="COMING SOON"><div style={{display:"grid",gridTemplateColumns:m?"repeat(4,1fr)":"repeat(8,1fr)",gap:m?5:8}}>{soon.slice(0,8).map((g,i)=><GC key={g.id} game={g} delay={i*25} onClick={setSel} mobile={m} userData={ud} size="tiny"/>)}</div></Sec>}

          <Sec title="TOP ACTION"><div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>{action.slice(0,6).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>
          <Sec title="TOP RPGs"><div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>{rpg.slice(0,6).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>
        </>}</div>}

      {/* ACTIVITY FEED */}
      {pg==="feed"&&user&&<div style={{animation:"fadeIn .3s",paddingTop:10}}>
        <h2 style={{fontFamily:"'Outfit'",fontSize:m?18:22,fontWeight:900,marginBottom:16}}>Activity</h2>
        {feed.length>0?<div style={{display:"flex",flexDirection:"column",gap:6}}>
          {feed.map(a=><div key={a.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:10,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)"}}>
            {a.game_img&&<img src={a.game_img} alt="" style={{width:36,height:20,borderRadius:4,objectFit:"cover",flexShrink:0}} onClick={()=>{const g=all.find(x=>x.id===a.game_id);if(g)setSel(g)}}/>}
            <div style={{flex:1,fontSize:12}}>
              <div style={{width:22,height:22,borderRadius:11,background:"linear-gradient(135deg,#00d4ff,#8b5cf6)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:800,marginRight:6,verticalAlign:"middle"}}>{(a.profiles?.display_name||"?").charAt(0).toUpperCase()}</div>
              <span style={{fontWeight:700}}>{a.profiles?.display_name||"User"}</span>
              <span style={{color:"#555"}}> {a.action} </span>
              {a.game_title&&<span style={{fontWeight:700,color:"#00d4ff"}}>{a.game_title}</span>}
              {a.action==="rated"&&a.rating&&<span style={{color:"#FFD700",marginLeft:4}}>{a.rating}★</span>}
            </div>
            <span style={{fontSize:9,color:"#333",flexShrink:0}}>{timeAgo(a.created_at)}</span>
          </div>)}</div>
        :<div style={{textAlign:"center",padding:50,color:"#555"}}><div style={{fontSize:36,marginBottom:8}}>⚡</div>No activity yet. Follow people to see their updates!</div>}</div>}
      ...
      {pg==="library"&&user&&<div style={{animation:"fadeIn .3s",paddingTop:10}}>
        ...
        {lib.length>0?<><div className="hs" style={{display:"flex",gap:5,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(auto-fill,minmax(140px,1fr))",gap:m?6:10}}>{flib.map((g,i)=><GC key={g.id} game={g} delay={i*25} onClick={setSel} mobile={m} userData={ud}/>)}</div>
        </>:<div style={{textAlign:"center",padding:50,color:"#555"}}><div style={{fontSize:36,marginBottom:8}}>📚</div>Empty library</div>}</div>}
      ...
      {pg==="profile"&&user&&<div style={{animation:"fadeIn .3s",paddingTop:10}}>
        <div style={{display:"flex",alignItems:m?"center":"flex-start",gap:m?14:20,marginBottom:24,flexDirection:m?"column":"row"}}>
          ...
          <div style={{textAlign:m?"center":"left",flex:1}}>
            ...
            <div style={{display:"flex",gap:18,marginTop:10,justifyContent:m?"center":"flex-start"}}>
              {[{v:lib.length,l:"Games"},{v:lib.filter(g=>ud[g.id]?.status==="completed").length,l:"Completed"},{v:fCounts.followers,l:"Followers"},{v:fCounts.following,l:"Following"}].map((s,i)=>
        ...
        <div style={{marginBottom:24}}>
          <h3 style={{fontSize:13,fontWeight:700,color:"#888",letterSpacing:".04em",marginBottom:10}}>MY LISTS</h3>
          {myLists.length>0&&<div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>{myLists.map(l=><div key={l.id} style={{padding:"10px 14px",borderRadius:10,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)"}}>
            <div style={{fontSize:13,fontWeight:700}}>📝 {l.title}</div><div style={{fontSize:10,color:"#555",marginTop:2}}>{l.game_ids?.length||0} games</div></div>)}</div>}
          <div style={{display:"flex",gap:6}}><input placeholder="New list..." value={newLN} onChange={e=>setNewLN(e.target.value)} onKeyDown={e=>e.key==="Enter"&&hcl()}
            ...
            <button onClick={hcl} style={{padding:"8px 14px",borderRadius:8,border:"none",background:"#00d4ff",color:"#000",fontSize:11,fontWeight:800,cursor:"pointer"}}>Create</button></div></div>
        {lib.length>0&&<><h3 style={{fontSize:13,fontWeight:700,color:"#888",letterSpacing:".04em",marginBottom:10}}>GAMES</h3>
          <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(auto-fill,minmax(140px,1fr))",gap:m?6:10}}>{lib.map((g,i)=><GC key={g.id} game={g} delay={i*25} onClick={setSel} mobile={m} userData={ud}/>)}</div></>}
      </div>}
    </main>
