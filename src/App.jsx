...
const REVIEWS = [
  { user:"GamerX", avatar:"😎", game:"Elden Ring", gid:1, rating:5, text:"A masterpiece that redefines open-world design.", likes:342, time:"2h" },
  { user:"NightOwl", avatar:"🦉", game:"Baldur's Gate 3", gid:2, rating:5, text:"Larian raised the bar for every RPG. 240 hours and I want more.", likes:218, time:"4h" },
  { user:"RetroKing", avatar:"👾", game:"Red Dead Redemption 2", gid:6, rating:5, text:"Arthur Morgan's story will stay with me forever.", likes:456, time:"5h" },
  { user:"ProSniper", avatar:"🎯", game:"Marvel Rivals", gid:8, rating:4, text:"Best hero shooter in years. Marvel IP makes it epic.", likes:89, time:"8h" },
  { user:"SoulsVet", avatar:"⚔️", game:"Sekiro", gid:7, rating:5, text:"Combat system is unmatched. Once it clicks, nothing compares.", likes:267, time:"12h" },
];
...
const GameCard = ({ game, onClick, wide, delay = 0, mobile }) => {
  ...
  return <div onClick={() => onClick?.(game)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
    ...
    {!err ? <img src={game.img} alt={game.title} onError={() => setErr(true)}
      ...
      : <div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#0f0c29,#302b63,#24243e)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 48, color: "#ffffff30" }}>🎮</div>}
    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top,rgba(0,0,0,.92) 0%,rgba(0,0,0,.3) 50%,transparent 100%)" }} />
    {game.status && <div style={{ position: "absolute", top: 8, left: 8, padding: "3px 8px", borderRadius: 20,
      background: "rgba(0,0,0,.55)", backdropFilter: "blur(8px)", border: "1px solid " + (STATUS_CFG[game.status]?.color || "#333") + "40",
      fontSize: 9, fontWeight: 800, color: STATUS_CFG[game.status]?.color, display: "flex", alignItems: "center", gap: 3 }}>
      {STATUS_CFG[game.status]?.icon} {STATUS_CFG[game.status]?.label}</div>}
    {game.completion > 0 && <div style={{ position: "absolute", top: 8, right: 8 }}><Ring pct={game.completion} size={mobile ? 32 : 36} stroke={2.5} /></div>}
    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: wide ? "16px 18px" : mobile ? "10px 12px" : "12px 14px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }}>
        ...
        <div style={{ display: "flex", gap: 3 }}>
          {game.platforms.slice(0, 3).map(p => <span key={p} style={{ fontSize: 8, fontWeight: 800, padding: "2px 5px", borderRadius: 3, background: "rgba(255,255,255,.1)", color: PLATFORMS[p]?.color }}>{p}</span>)}
        </div>
      </div>
    </div>
  </div>;
...
const Modal = ({ game, onClose, mobile }) => {
      ...
      <div style={{ position: "relative", height: mobile ? 180 : 220, overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", bottom: 14, left: mobile ? 16 : 24, right: mobile ? 16 : 24, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div><h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 900, margin: 0, textShadow: "0 4px 16px rgba(0,0,0,.8)" }}>{game.title}</h2>
            <div style={{ color: "rgba(255,255,255,.45)", fontSize: 12, marginTop: 2 }}>{game.year} · {game.genre}{game.reviews > 0 && " · " + (game.reviews/1000).toFixed(1) + "k reviews"}</div></div>
          {game.rating && <div style={{ textAlign: "center", flexShrink: 0 }}><div style={{ fontSize: mobile ? 28 : 36, fontWeight: 900, color: "#FFD700", lineHeight: 1 }}>{game.rating}</div><Stars rating={Math.round(game.rating)} size={10} /></div>}
        </div>
      </div>
      <div style={{ padding: mobile ? "14px 16px 28px" : "18px 24px 28px" }}>
        <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
          {game.platforms.map(p => <span key={p} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700,
            background: PLATFORMS[p]?.color + "10", color: PLATFORMS[p]?.color, border: "1px solid " + PLATFORMS[p]?.color + "28" }}>{PLATFORMS[p]?.icon} {p}</span>)}
        </div>
        <div style={{ padding: mobile ? 14 : 18, borderRadius: 14, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.05)", marginBottom: 18 }}>
          ...
          <Stars rating={mr} size={mobile ? 24 : 26} interactive onRate={setMr} />
          <div style={{ fontSize: 10, color: "#555", fontWeight: 700, letterSpacing: ".08em", marginTop: 14, marginBottom: 6 }}>STATUS</div>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,.05)", marginBottom: 16 }}>
        ...
        {tab === "overview" && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          {[{ l: "PLAYTIME", v: game.playtime + "h", c: "#00d4ff" }, { l: "COMPLETION", ring: true }, { l: "MY RATING", v: mr ? mr + "/5" : "—", c: "#FFD700" }].map((s, i) =>
            <div key={i} style={{ padding: 12, borderRadius: 12, background: "rgba(255,255,255,.025)", textAlign: "center" }}>
              <div style={{ fontSize: 10, color: "#444", fontWeight: 700 }}>{s.l}</div>
              {s.ring ? <div style={{ marginTop: 6, display: "flex", justifyContent: "center" }}><Ring pct={game.completion || 0} size={mobile ? 44 : 52} stroke={4} /></div>
          ...
          {REVIEWS.filter(r => r.gid === game.id).length > 0 ? REVIEWS.filter(r => r.gid === game.id).map((r, i) =>
            <div key={i} style={{ padding: 14, borderRadius: 12, background: "rgba(255,255,255,.025)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: 22 }}>{r.avatar}</span><span style={{ fontWeight: 700, fontSize: 13 }}>{r.user}</span>
                <div style={{ marginLeft: "auto" }}><Stars rating={r.rating} size={10} /></div></div>
              <p style={{ color: "#888", fontSize: 12.5, lineHeight: 1.6, margin: 0 }}>{r.text}</p>
              <div style={{ display: "flex", gap: 12, marginTop: 8, fontSize: 11, color: "#444" }}><span>❤️ {r.likes}</span><span>💬 Reply</span></div>
            </div>) : <div style={{ textAlign: "center", padding: 36, color: "#444" }}><div style={{ fontSize: 32, marginBottom: 8 }}>💬</div><div style={{ fontSize: 13 }}>No reviews yet</div></div>}
        </div>}
      </div>
    </div>
  </div>;
...
export default function App() {
  ...
  const NAV = [{ id: "home", icon: "🏠", l: "Home" }, { id: "library", icon: "📚", l: "Library" }, { id: "explore", icon: "🔍", l: "Explore" }, { id: "stats", icon: "📊", l: "Stats" }, { id: "lists", icon: "📝", l: "Lists" }];
  ...
  return <div style={{ fontFamily: "'DM Sans','Outfit',system-ui,sans-serif", background: "#08080f", color: "#fff", minHeight: "100vh" }}>
    ...
    {!mobile && <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,8,15,.82)", backdropFilter: "blur(24px) saturate(1.4)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
        <div onClick={() => setPage("home")} style={{ display: "flex", alignItems: "center", gap: 7, cursor: "pointer" }}>
          ...
          <span style={{ fontFamily: "'Outfit'", fontSize: 17, fontWeight: 900, letterSpacing: ".04em", background: "linear-gradient(135deg,#00d4ff,#8b5cf6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Gameboxed</span>
        </div>
        <div style={{ display: "flex", gap: 2 }}>{NAV.map(n => <button key={n.id} onClick={() => { setPage(n.id); setF("all"); }} style={{
          ...
          <span style={{ fontSize: 12 }}>{n.icon}</span>{n.l}</button>)}</div>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ position: "relative" }}><input placeholder="Search..." value={search} onChange={e => { setSearch(e.target.value); if (e.target.value) setPage("library"); }}
          style={{ padding: "6px 10px 6px 30px", borderRadius: 8, border: "1px solid rgba(255,255,255,.06)", background: "rgba(255,255,255,.04)", color: "#fff", fontSize: 12, width: 160, outline: "none" }} />
          <span style={{ position: "absolute", left: 9, top: "50%", transform: "translateY(-50%)", fontSize: 12, color: "#444" }}>🔍</span></div>
        <div style={{ width: 30, height: 30, borderRadius: 7, background: "linear-gradient(135deg,#00d4ff,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>M</div>
      </div>
    </nav>}
    ...
    {mobile && <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,8,15,.9)", backdropFilter: "blur(20px)",
      ...
      {searchOpen ? <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
        <input autoFocus placeholder="Search games..." value={search} onChange={e => { setSearch(e.target.value); if (e.target.value) setPage("library"); }}
        ...
        <span onClick={() => { setSearchOpen(false); setSearch(""); }} style={{ color: "#00d4ff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Cancel</span>
      </div> : <>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <span style={{ fontFamily: "'Outfit'", fontSize: 16, fontWeight: 900, letterSpacing: ".04em", background: "linear-gradient(135deg,#00d4ff,#8b5cf6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Gameboxed</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span onClick={() => setSearchOpen(true)} style={{ fontSize: 18, cursor: "pointer" }}>🔍</span>
          <div style={{ width: 28, height: 28, borderRadius: 7, background: "linear-gradient(135deg,#00d4ff,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800 }}>M</div>
        </div>
      </>}
    </div>}
    ...
    <main style={{ maxWidth: 1200, margin: "0 auto", padding: mobile ? "12px 14px 100px" : "20px 18px 80px" }}>
      ...
      {page === "home" && <div style={{ animation: "fadeIn .4s" }}>
        <div style={{ borderRadius: mobile ? 18 : 22, padding: mobile ? "28px 22px" : "44px 36px", marginBottom: mobile ? 24 : 36,
          ...
          <p style={{ color: "#666", fontSize: mobile ? 13 : 14, maxWidth: 460, lineHeight: 1.7, marginBottom: mobile ? 18 : 24, position: "relative" }}>Your personal gaming diary. Rate, review, and build your ultimate collection.</p>
          <div style={{ display: "flex", gap: 10, position: "relative" }}>
            ...
            <button onClick={() => setPage("explore")} style={{ padding: "10px 22px", borderRadius: 10, border: "1px solid rgba(255,255,255,.08)", background: "rgba(255,255,255,.03)", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Explore</button>
          </div>
        </div>

        <Section title="🎮 Now Playing" action="View All →" onAction={() => { setF("playing"); setPage("library"); }} mobile={mobile}>
          <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
            {GAMES.filter(g => g.status === "playing").map((g, i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i * 80} onClick={setSel} mobile={mobile} /></div>)}
          </div>
        </Section>

        <Section title="⭐ Featured" mobile={mobile}>
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: mobile ? 10 : 14 }}>
            {GAMES.filter(g => g.rating >= 4.8).slice(0, 2).map((g, i) => <GameCard key={g.id} game={g} wide delay={i * 100} onClick={setSel} mobile={mobile} />)}
          </div>
        </Section>

        <Section title="⚡ Activity" mobile={mobile}>
          <div style={{ borderRadius: 14, overflow: "hidden", border: "1px solid rgba(255,255,255,.04)", background: "rgba(255,255,255,.015)" }}>
            {[{ u: "You", a: "started playing", g: "Ghost of Tsushima", t: "2h" }, { u: "AhmadK", a: "completed", g: "Elden Ring", t: "3h" },
              ...
              return <div key={i} style={{ padding: mobile ? "9px 12px" : "10px 16px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid rgba(255,255,255,.03)" }}>
                {gm && <img src={gm.img} alt="" style={{ width: 40, height: 20, borderRadius: 3, objectFit: "cover" }} />}
                <div style={{ flex: 1, fontSize: mobile ? 11 : 12 }}><span style={{ color: "#00d4ff", fontWeight: 700 }}>{a.u}</span><span style={{ color: "#555" }}> {a.a} </span><span style={{ fontWeight: 700 }}>{a.g}</span></div>
                <span style={{ fontSize: 10, color: "#333" }}>{a.t}</span></div>; })}
          </div>
        </Section>

        <Section title="💬 Popular Reviews" mobile={mobile}>
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(auto-fill,minmax(280px,1fr))", gap: 12 }}>
            {REVIEWS.slice(0, mobile ? 3 : 5).map((r, i) => { const gm = GAMES.find(g => g.id === r.gid);
              return <div key={i} style={{ borderRadius: 14, overflow: "hidden", background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.04)" }}>
                {gm && <div style={{ position: "relative", height: 60, overflow: "hidden" }}><img src={gm.img} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: .45 }} />
                  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom,transparent,rgba(8,8,15,.9))" }} />
                  <span style={{ position: "absolute", bottom: 6, left: 12, fontSize: 11, fontWeight: 700, color: "#00d4ff" }}>{r.game}</span></div>}
                <div style={{ padding: "10px 12px 12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}><span style={{ fontSize: 20 }}>{r.avatar}</span><span style={{ fontWeight: 700, fontSize: 12 }}>{r.user}</span>
                    <div style={{ marginLeft: "auto" }}><Stars rating={r.rating} size={10} /></div></div>
      ...
      {page === "library" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        <div className="hide-scroll" style={{ display: "flex", gap: 5, marginBottom: 10, overflowX: "auto", paddingBottom: 8 }}>
          {[["all","All"],["playing","Playing"],["completed","Done"],["backlog","Backlog"],["wishlist","Wishlist"],["dropped","Dropped"]].map(([k,l]) =>
            <button key={k} onClick={() => setF(k)} style={{ padding: "5px 13px", borderRadius: 18, fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap",
              border: filter === k ? "1px solid #00d4ff" : "1px solid rgba(255,255,255,.06)", background: filter === k ? "#00d4ff14" : "transparent",
              color: filter === k ? "#00d4ff" : "#555" }}>{l} <span style={{ opacity: .5 }}>{k === "all" ? GAMES.length : GAMES.filter(g => g.status === k).length}</span></button>)}
        </div>
        <div className="hide-scroll" style={{ display: "flex", gap: 5, marginBottom: 18, overflowX: "auto", paddingBottom: 4 }}>
          {Object.entries(PLATFORMS).map(([p, d]) => <button key={p} onClick={() => setF(filter === p ? "all" : p)} style={{ padding: "5px 10px", borderRadius: 18, fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap",
            border: filter === p ? "1px solid " + d.color : "1px solid rgba(255,255,255,.06)", background: filter === p ? d.color + "14" : "transparent", color: filter === p ? d.color : "#555" }}>{d.icon} {p}</button>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(auto-fill,minmax(220px,1fr))", gap: mobile ? 10 : 14 }}>
          {filtered.map((g, i) => <GameCard key={g.id} game={g} delay={i * 50} onClick={setSel} mobile={mobile} />)}
        </div>
        {!filtered.length && <div style={{ textAlign: "center", padding: 60, color: "#444" }}><div style={{ fontSize: 48, marginBottom: 10 }}>🎮</div><div style={{ fontSize: 15, fontWeight: 700 }}>No games found</div></div>}
      </div>}
      ...
      {page === "explore" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        <div className="hide-scroll" style={{ display: "flex", gap: 7, marginBottom: 28, overflowX: "auto", paddingBottom: 4 }}>
          {["Action","RPG","Action RPG","FPS","Horror"].map(g => <button key={g} onClick={() => { setF(g); setPage("library"); }}
            style={{ padding: "7px 16px", borderRadius: 18, whiteSpace: "nowrap", border: "1px solid rgba(255,255,255,.06)", background: "rgba(255,255,255,.03)", color: "#bbb", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>{g}</button>)}
        </div>
        <Section title="⭐ Top Rated" mobile={mobile}>
          <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
            {[...GAMES].filter(g => g.rating).sort((a,b) => b.rating - a.rating).slice(0,6).map((g,i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i*80} onClick={setSel} mobile={mobile} /></div>)}
          </div>
        </Section>
        <Section title="🕐 Most Played" mobile={mobile}>
          <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
            {[...GAMES].sort((a,b) => b.playtime - a.playtime).slice(0,6).map((g,i) => <div key={g.id} style={{ minWidth: mobile ? 200 : 230, flex: "0 0 auto" }}><GameCard game={g} delay={i*80} onClick={setSel} mobile={mobile} /></div>)}
          </div>
        </Section>
      </div>}
      ...
      {page === "stats" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        {(() => { const comp = GAMES.filter(g => g.status === "completed"); const totalH = GAMES.reduce((s,g) => s + g.playtime, 0);
          const avgR = (comp.filter(g => g.myRating).reduce((s,g) => s + g.myRating, 0) / comp.filter(g => g.myRating).length).toFixed(1);
          const gc = {}; GAMES.forEach(g => { gc[g.genre] = (gc[g.genre]||0)+1 }); const gcs = Object.entries(gc).sort((a,b)=>b[1]-a[1]); const mx = gcs[0]?.[1]||1;
          ...
          return <>
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(auto-fit,minmax(140px,1fr))", gap: 10, marginBottom: 28 }}>
              {[{l:"Games",v:GAMES.length,i:"🎮",c:"#00d4ff"},{l:"Completed",v:comp.length,i:"✅",c:"#00ff88"},{l:"Total Hours",v:totalH.toLocaleString(),i:"⏱️",c:"#8b5cf6"},{l:"Avg Rating",v:avgR+"★",i:"⭐",c:"#FFD700"}].map((s,i) =>
                <div key={i} style={{ padding: mobile ? 14 : 18, borderRadius: 14, textAlign: "center", background: "linear-gradient(135deg,"+s.c+"08,"+s.c+"04)", border: "1px solid "+s.c+"18" }}>
                  ...
                  <div style={{ fontSize: 10, color: "#555", fontWeight: 700, marginTop: 2 }}>{s.l}</div></div>)}
            </div>
            <h3 style={{ fontFamily: "'Outfit'", fontSize: 17, fontWeight: 800, marginBottom: 14 }}>Genre Breakdown</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 28 }}>
              {gcs.map(([g,c],i) => <div key={g} style={{ display: "flex", alignItems: "center", gap: mobile ? 8 : 12 }}>
                <span style={{ width: mobile ? 70 : 85, fontSize: 12, color: "#777", fontWeight: 600, textAlign: "right" }}>{g}</span>
                <div style={{ flex: 1, height: 24, background: "rgba(255,255,255,.03)", borderRadius: 6, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: (c/mx)*100+"%", background: "linear-gradient(90deg,"+cols[i%cols.length]+","+cols[i%cols.length]+"66)",
                    borderRadius: 6, display: "flex", alignItems: "center", paddingLeft: 8, transition: "width 1s cubic-bezier(.22,1,.36,1)" }}>
                    <span style={{ fontSize: 10, fontWeight: 800 }}>{c}</span></div></div></div>)}
            </div>
            <h3 style={{ fontFamily: "'Outfit'", fontSize: 17, fontWeight: 800, marginBottom: 14 }}>Platforms</h3>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(80px,1fr))", gap: 10, marginBottom: 28 }}>
              {Object.entries(PLATFORMS).map(([n,d]) => { const c = GAMES.filter(g => g.platforms.includes(n)).length;
                return <div key={n} style={{ padding: mobile ? 14 : 18, borderRadius: 14, textAlign: "center", background: d.color+"08", border: "1px solid "+d.color+"18" }}>
                  <div style={{ fontSize: mobile ? 22 : 26 }}>{d.icon}</div><div style={{ fontSize: mobile ? 18 : 22, fontWeight: 900, color: d.color, marginTop: 3 }}>{c}</div>
                  <div style={{ fontSize: 11, color: "#555", fontWeight: 700 }}>{n}</div></div>; })}
            </div>
            ...
            <div className="hide-scroll" style={{ display: "flex", gap: 16, overflowX: "auto", paddingBottom: 4 }}>
              {GAMES.filter(g => g.completion > 0).sort((a,b) => b.completion - a.completion).map(g =>
                <div key={g.id} style={{ textAlign: "center", flexShrink: 0 }}><Ring pct={g.completion} size={mobile ? 56 : 64} stroke={4} />
                  <div style={{ fontSize: 10, color: "#555", marginTop: 4, maxWidth: 64, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.title}</div></div>)}
            </div></>;
        })()}
      </div>}
      ...
      {page === "lists" && <div style={{ animation: "fadeIn .4s" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <h2 style={{ fontFamily: "'Outfit'", fontSize: mobile ? 22 : 26, fontWeight: 900 }}>📝 Lists</h2>
          <button style={{ padding: "8px 16px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#00d4ff,#0088bb)", color: "#fff", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>+ Create</button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(auto-fill,minmax(260px,1fr))", gap: 12 }}>
          {LISTS.map((l,i) => { const imgs = l.gids.map(id => GAMES.find(g => g.id === id)).filter(Boolean);
            return <div key={i} style={{ borderRadius: 16, overflow: "hidden", background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.04)", cursor: "pointer" }}>
              <div style={{ display: "flex", height: 65, overflow: "hidden" }}>{imgs.map((g,j) => <img key={j} src={g.img} alt="" style={{ flex: 1, objectFit: "cover", opacity: .6 }} />)}</div>
              <div style={{ padding: "12px 14px" }}><div style={{ fontSize: 14, fontWeight: 800, marginBottom: 3 }}>{l.emoji} {l.title}</div>
                <div style={{ fontSize: 11, color: "#555" }}>{l.count} games · by <span style={{ color: "#00d4ff" }}>{l.author}</span></div></div></div>; })}
        </div>
      </div>}
    </main>
    ...
    {mobile && <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 90, background: "rgba(8,8,15,.92)", backdropFilter: "blur(20px)",
      ...
      {NAV.map(n => <div key={n.id} onClick={() => { setPage(n.id); setF("all"); }}
        ...
        {page === n.id && <div style={{ width: 4, height: 4, borderRadius: 2, background: "#00d4ff", marginTop: -1 }} />}
      </div>)}
    </div>}

    {sel && <Modal game={sel} onClose={() => setSel(null)} mobile={mobile} />}
  </div>;
}
