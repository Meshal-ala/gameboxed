...
const SC={completed:{c:"#6ee7b7",l:"Completed",g:"from-emerald-500"},playing:{c:"#67e8f9",l:"Playing",g:"from-cyan-500"},wishlist:{c:"#fde68a",l:"Wishlist",g:"from-amber-300"},dropped:{c:"#fda4af",l:"Dropped",g:"from-rose-400"},backlog:{c:"#c4b5fd",l:"Backlog",g:"from-violet-400"}};
...
const upAv=async(uid,file)=>{const p=`${uid}/av_${Date.now()}.${file.name.split(".").pop()}`;const{error}=await supabase.storage.from("avatars").upload(p,file,{cacheControl:"0",upsert:false});if(error)throw error;await supabase.from("profiles").update({avatar_url:p}).eq("id",uid);return p};
...
const glassHov={...glass,border:"1px solid rgba(255,255,255,.12)"};
...
const Stars=({rating=0,size=14,interactive,onRate,show})=>{const[h,setH]=useState(0);const a=h||rating;
  ...
  return<div style={{display:"flex",gap:1,alignItems:"center"}}>{[1,2,3,4,5].map(s=>{const f=s<=Math.floor(a),hf=!f&&s-.5<=a&&s>a-.5;
    return<span key={s} onClick={e=>c(s,e)} onMouseEnter={()=>interactive&&setH(s)} onMouseLeave={()=>interactive&&setH(0)}
      style={{fontSize:size,cursor:interactive?"pointer":"default",position:"relative",lineHeight:1,color:f||hf?"#fde68a":"rgba(255,255,255,.1)",transition:"all .12s",transform:interactive&&s<=Math.ceil(h)?"scale(1.15)":"scale(1)"}}>
      {hf?<><span style={{position:"absolute",overflow:"hidden",width:"50%"}}>★</span><span style={{color:"rgba(255,255,255,.1)"}}>★</span></>:"★"}</span>})}
...
const GC=({game:g,onClick,delay=0,mobile:m,ud,big})=>{const[hov,setHov]=useState(false);const[vis,setVis]=useState(false);const[err,setErr]=useState(false);
  ...
  return<div onClick={()=>onClick?.(g)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    ...
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:big?"20px 24px":"10px 12px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:big?6:3}}>
        <span style={{fontSize:big?12:8,color:"rgba(255,255,255,.4)"}}>{g.y}</span>
        {big&&<><span style={{color:"rgba(255,255,255,.15)"}}>·</span><span style={{fontSize:12,color:"rgba(255,255,255,.4)"}}>{g.genre}</span></>}
        {big&&g.mc&&<><span style={{color:"rgba(255,255,255,.15)"}}>·</span><span style={{padding:"2px 6px",borderRadius:4,background:g.mc>=75?"#6ee7b720":"#fde68a20",fontSize:10,fontWeight:800,color:g.mc>=75?"#6ee7b7":"#fde68a"}}>MC {g.mc}</span></>}
      </div>
      {!big&&<div style={{display:"flex",gap:3,marginTop:3}}>{g.pf.slice(0,2).map((p,i)=><span key={i} style={{fontSize:6,fontWeight:800,padding:"1px 4px",borderRadius:4,background:"rgba(255,255,255,.06)",color:p.c}}>{p.n}</span>)}</div>}
    </div></div>};

/* Tiny card for Coming Soon */
const TC=({game:g,onClick,delay=0,m,ud})=>{const[vis,setVis]=useState(false);
  ...
  return<div onClick={()=>onClick?.(g)} style={{opacity:vis?1:0,transform:vis?"none":"translateY(6px)",transition:"all .2s",cursor:"pointer",textAlign:"center"}}>
    <div style={{borderRadius:10,overflow:"hidden",aspectRatio:"1",marginBottom:4,position:"relative",boxShadow:"0 4px 12px rgba(0,0,0,.2)"}}>
      ...
      <div style={{position:"absolute",bottom:3,left:0,right:0,textAlign:"center"}}><span style={{fontSize:8,color:"#fde68a",fontWeight:800}}>{g.y}</span></div></div>
    <div style={{fontSize:9,fontWeight:600,lineHeight:1.2,color:"rgba(255,255,255,.6)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.t}</div></div>};
...
const Auth=({onClose,onAuth})=>{const[mode,setMode]=useState("login");const[email,setEmail]=useState("");const[pw,setPw]=useState("");const[name,setName]=useState("");const[err,setErr]=useState("");const[ld,setLd]=useState(false);const[sent,setSent]=useState(false);
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(15,12,25,.95)",backdropFilter:"blur(24px)",display:"flex",alignItems:"center",justifyContent:"center",animation:"fadeIn .15s",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{...glass,borderRadius:24,width:"100%",maxWidth:400,padding:"40px 32px",background:"rgba(30,27,46,.8)",animation:"slideUp .25s ease"}}>
      {sent?<div style={{textAlign:"center"}}><div style={{fontSize:48,marginBottom:16}}>✉️</div><h2 style={{fontSize:22,fontWeight:800}}>Check your inbox</h2><p style={{color:"rgba(255,255,255,.5)",fontSize:14,marginTop:8}}>We sent a link to <span style={{color:"#67e8f9"}}>{email}</span></p>
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:1800,background:"rgba(15,12,25,.95)",display:"flex",alignItems:m?"flex-end":"center",justifyContent:"center",padding:m?0:16}}>
    <div onClick={e=>e.stopPropagation()} style={{...glass,background:"rgba(30,27,46,.9)",width:"100%",maxWidth:m?"100%":400,maxHeight:m?"80vh":"70vh",borderRadius:m?"24px 24px 0 0":24,overflow:"auto"}}>
      ...
      <div style={{padding:"20px 20px 24px"}}><h3 style={{fontSize:18,fontWeight:900,marginBottom:16}}>{type==="followers"?"Followers":"Following"}</h3>
        {ld?<Loader/>:list.length?list.map(p=><div key={p.id} onClick={()=>{onClose();setTimeout(()=>setVU(p.id),50)}} style={{display:"flex",alignItems:"center",gap:12,padding:"10px",borderRadius:12,cursor:"pointer",marginBottom:4,transition:"background .15s"}}
...
const UP=({viewId,onClose,me,m,onFollow,setVU})=>{
  ...
  if(ld)return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:1500,background:"rgba(15,12,25,.95)",display:"flex",alignItems:"center",justifyContent:"center"}}><Loader/></div>;
  return<><div onClick={onClose} style={{position:"fixed",inset:0,zIndex:1500,background:"rgba(15,12,25,.95)",backdropFilter:"blur(16px)",display:"flex",alignItems:m?"flex-end":"center",justifyContent:"center",padding:m?0:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#16132a",width:"100%",maxWidth:m?"100%":520,maxHeight:m?"92vh":"90vh",borderRadius:m?"24px 24px 0 0":24,overflow:"auto",border:"1px solid rgba(255,255,255,.06)"}}>
      {m&&<div onClick={onClose} style={{display:"flex",justifyContent:"center",padding:"12px 0 0"}}><div style={{width:32,height:4,borderRadius:2,background:"rgba(255,255,255,.1)"}}/></div>}
      {/* Cover */}
      <div style={{height:100,background:"linear-gradient(135deg,#67e8f930,#818cf830,#c4b5fd30)",position:"relative"}}>
        <div style={{position:"absolute",bottom:-30,left:24}}><Av url={p?.avatar_url} name={p?.display_name} size={64} v={Date.now()}/></div></div>
      <div style={{padding:"40px 24px 24px"}}>
        <h2 style={{fontSize:22,fontWeight:900}}>{p?.display_name||"User"}</h2>
        {p?.username&&<div style={{color:"rgba(255,255,255,.3)",fontSize:13}}>@{p.username}</div>}
        {p?.bio&&<p style={{color:"rgba(255,255,255,.5)",fontSize:13,lineHeight:1.6,marginTop:6}}>{p.bio}</p>}
        <div style={{display:"flex",gap:20,marginTop:12}}>
          <div><span style={{fontWeight:900}}>{gs.length}</span> <span style={{color:"rgba(255,255,255,.3)",fontSize:12}}>games</span></div>
        ...
        {me&&me.id!==viewId&&<button onClick={tog} style={{marginTop:12,padding:"9px 28px",borderRadius:12,border:isF?"1px solid rgba(255,255,255,.1)":"none",background:isF?"transparent":"linear-gradient(135deg,#67e8f9,#818cf8)",color:isF?"rgba(255,255,255,.4)":"#0f0c19",fontSize:13,fontWeight:800,cursor:"pointer"}}>{isF?"Following ✓":"Follow"}</button>}

        {/* Top 5 */}
        {top5.length>0&&<div style={{marginTop:20}}><h4 style={{fontSize:12,fontWeight:800,color:"#67e8f9",marginBottom:10}}>⭐ TOP RATED</h4>
          <div style={{display:"flex",gap:8}} className="hs">{top5.map((g,i)=>
            <div key={g.game_id} style={{minWidth:70,textAlign:"center",flexShrink:0}}>
              <div style={{position:"relative",borderRadius:10,overflow:"hidden",aspectRatio:"2/3",marginBottom:4}}>
                ...
                <div style={{position:"absolute",top:3,left:3,background:"linear-gradient(135deg,#67e8f9,#818cf8)",borderRadius:6,padding:"1px 6px",fontSize:9,fontWeight:900,color:"#0f0c19"}}>#{i+1}</div></div>
              <div style={{fontSize:9,fontWeight:600,lineHeight:1.2}}>{g.game_title}</div>
              <span style={{fontSize:9,color:"#fde68a",fontWeight:800}}>★ {g.my_rating}</span></div>)}</div></div>}

        {/* Activity */}
        {acts.length>0&&<div style={{marginTop:20}}><h4 style={{fontSize:12,fontWeight:800,color:"#67e8f9",marginBottom:10}}>⚡ ACTIVITY</h4>
          {acts.slice(0,6).map(a=><div key={a.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,.03)",fontSize:12}}>
            {a.game_img&&<img src={a.game_img} style={{width:28,height:16,borderRadius:4,objectFit:"cover"}}/>}
            ...
            <span style={{marginLeft:"auto",color:"rgba(255,255,255,.15)",fontSize:10}}>{tA(a.created_at)}</span></div>)}</div>}
      </div></div></div>
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(15,12,25,.95)",backdropFilter:"blur(16px)",display:"flex",alignItems:m?"flex-end":"center",justifyContent:"center",animation:"fadeIn .12s",padding:m?0:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#16132a",width:"100%",maxWidth:m?"100%":660,maxHeight:m?"93vh":"88vh",borderRadius:m?"24px 24px 0 0":24,overflow:"auto",border:"1px solid rgba(255,255,255,.06)"}}>
      ...
      <div style={{position:"relative",height:m?180:240,overflow:"hidden"}}><img src={g.img} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
        ...
        <div style={{position:"absolute",bottom:16,left:m?16:24}}>
          <h2 style={{fontFamily:"'Outfit'",fontSize:m?24:32,fontWeight:900}}>{g.t}</h2>
      ...
      <div style={{padding:m?"14px 16px 36px":"20px 24px 28px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:18}}>
          ...
          <div style={{...glass,padding:14,borderRadius:14}}><div style={{fontSize:9,color:"rgba(255,255,255,.3)",fontWeight:700,marginBottom:8}}>STATUS</div>
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{Object.entries(SC).map(([k,c])=><button key={k} onClick={()=>sv("status",k)} style={{padding:"4px 10px",borderRadius:20,fontSize:9,fontWeight:700,cursor:"pointer",border:"none",background:st===k?c.c:"rgba(255,255,255,.04)",color:st===k?"#0f0c19":"rgba(255,255,255,.3)",transition:"all .15s"}}>{c.l}</button>)}</div></div></div>

        <div style={{display:"flex",gap:4,marginBottom:16}}>{g.pf.map((p,i)=><span key={i} style={{fontSize:10,fontWeight:700,padding:"4px 10px",borderRadius:20,...glass,color:p.c}}>{p.n}</span>)}</div>
  ...
  return<div className="gbx" style={{fontFamily:"'DM Sans','Outfit',system-ui,sans-serif",color:"#fff",minHeight:"100vh"}}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,100..1000&family=Outfit:wght@100..900&display=swap');
      *{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:rgba(255,255,255,.08);border-radius:2px}
      ...
      .gbx{background:#0f0c19;background-image:radial-gradient(ellipse 70% 50% at 20% 0%,rgba(103,232,249,.06),transparent),radial-gradient(ellipse 50% 40% at 80% 10%,rgba(129,140,248,.06),transparent),radial-gradient(ellipse 60% 50% at 50% 100%,rgba(196,181,253,.04),transparent);background-attachment:fixed;min-height:100vh}
    ...
    {!m&&<nav style={{position:"sticky",top:0,zIndex:100,background:"rgba(15,12,25,.8)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,.04)",padding:"0 28px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",alignItems:"center",gap:24}}>
        <span onClick={()=>{setPg("home");setQ("")}} style={{fontFamily:"'Outfit'",fontSize:18,fontWeight:900,cursor:"pointer",background:"linear-gradient(135deg,#67e8f9,#818cf8)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>gameboxed</span>
        <div style={{display:"flex",gap:2}}>{NAV.filter(n=>n.id!=="profile").map(n=><button key={n.id} onClick={()=>{setPg(n.id);setQ("")}} style={{padding:"6px 14px",borderRadius:20,border:"none",background:pg===n.id?"rgba(255,255,255,.06)":"transparent",color:pg===n.id?"#fff":"rgba(255,255,255,.3)",cursor:"pointer",fontSize:12,fontWeight:700,transition:"all .15s"}}>{n.l}</button>)}</div></div>
    ...
    {m&&<div style={{position:"sticky",top:0,zIndex:100,background:"rgba(15,12,25,.85)",backdropFilter:"blur(20px)",padding:"0 14px",height:48,display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
      ...
      :<><span style={{fontFamily:"'Outfit'",fontSize:16,fontWeight:900,background:"linear-gradient(135deg,#67e8f9,#818cf8)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>gameboxed</span>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span onClick={()=>{setQO(true);setPg("search")}} style={{fontSize:15,cursor:"pointer",color:"rgba(255,255,255,.3)"}}>🔍</span>
    ...
    <main style={{maxWidth:1100,margin:"0 auto",padding:m?"8px 12px 86px":"16px 28px 44px"}}>
      ...
      {pg==="search"&&<div style={{animation:"fadeIn .15s"}}>
        ...
        {sng?<Loader/>:<>
          ...
          {sT==="people"&&(pSr.length?<div style={{display:"flex",flexDirection:"column",gap:8}}>{pSr.map(p=>
            <div key={p.id} onClick={()=>setVU(p.id)} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",borderRadius:16,...glass,cursor:"pointer",transition:"border .2s"}}
              ...
              <span style={{color:"#67e8f9",fontSize:12}}>View →</span></div>)}</div>:q&&<p style={{textAlign:"center",padding:40,color:"rgba(255,255,255,.15)"}}>No people found</p>)}
          {sT==="lists"&&(lSr.length?<div style={{display:"flex",flexDirection:"column",gap:8}}>{lSr.map(l=><div key={l.id} style={{padding:"14px 16px",borderRadius:16,...glass}}>
            <div style={{fontSize:15,fontWeight:700}}>📝 {l.title}</div><div style={{fontSize:11,color:"rgba(255,255,255,.25)",marginTop:4}}>{l.game_ids?.length||0} games · {l.profiles?.display_name}</div></div>)}</div>:q&&<p style={{textAlign:"center",padding:40,color:"rgba(255,255,255,.15)"}}>No lists found</p>)}
        </>}</div>}
      ...
      {pg==="home"&&<div style={{animation:"fadeIn .2s"}}>
        ...
        {ld?<Loader/>:<div style={{display:m?"block":"grid",gridTemplateColumns:"1fr 300px",gap:24}}>
          <div>
            {/* Bento hero */}
            {pop[0]&&<div style={{marginBottom:m?16:24}}>
              <GC game={pop[0]} onClick={setSel} mobile={m} ud={ud} big delay={0}/>
            </div>}

            <div className="sec-title">POPULAR THIS WEEK</div>
            {secGrid(pop.slice(1),5)}
            <div style={{height:24}}/>

            <div className="sec-title">JUST RELEASED</div>
            {secGrid(fresh,5)}
            <div style={{height:24}}/>

            {best[0]&&<div style={{marginBottom:24}}><GC game={best[0]} onClick={setSel} mobile={m} ud={ud} big delay={0}/></div>}

            <div className="sec-title">ALL-TIME BEST</div>
            {secGrid(best.slice(1),5)}
            <div style={{height:24}}/>

            <div className="sec-title">COMING SOON</div>
            <div style={{display:"grid",gridTemplateColumns:m?"repeat(4,1fr)":"repeat(7,1fr)",gap:m?8:10,marginBottom:24}}>{soon.slice(0,m?8:7).map((g,i)=><TC key={g.id} game={g} onClick={setSel} delay={i*15} m={m} ud={ud}/>)}</div>

            <div className="sec-title">ACTION</div>
            {secGrid(act,5)}<div style={{height:24}}/>
            <div className="sec-title">RPG</div>
            {secGrid(rpg,5)}<div style={{height:24}}/>
            <div className="sec-title">INDIE</div>
            {secGrid(indie,5)}
          </div>
      ...
      {pg==="profile"&&user&&<div style={{animation:"fadeIn .15s"}}>
        {/* Cover gradient */}
        <div style={{height:m?80:120,borderRadius:20,background:"linear-gradient(135deg,rgba(103,232,249,.15),rgba(129,140,248,.15),rgba(196,181,253,.15))",marginBottom:m?-30:-40,position:"relative"}}/>
        <div style={{display:"flex",alignItems:m?"center":"flex-start",gap:m?14:20,marginBottom:24,flexDirection:m?"column":"row",position:"relative",padding:m?"0 12px":"0 20px"}}>
          ...
          <div style={{textAlign:m?"center":"left",flex:1}}>
            ...
            {prof?.username&&<div style={{color:"rgba(255,255,255,.25)",fontSize:13,marginTop:2}}>@{prof.username}</div>}
            {prof?.bio&&<p style={{color:"rgba(255,255,255,.4)",fontSize:14,lineHeight:1.6,marginTop:6}}>{prof.bio}</p>}
            <div style={{display:"flex",gap:16,marginTop:10,justifyContent:m?"center":"flex-start"}}>
              <div><span style={{fontWeight:900}}>{lib.length}</span> <span style={{color:"rgba(255,255,255,.2)",fontSize:12}}>games</span></div>
              <div onClick={()=>setFlM("followers")} style={{cursor:"pointer"}}><span style={{fontWeight:900}}>{fc.followers}</span> <span style={{color:"#67e8f9",fontSize:12}}>followers</span></div>
              <div onClick={()=>setFlM("following")} style={{cursor:"pointer"}}><span style={{fontWeight:900}}>{fc.following}</span> <span style={{color:"#67e8f9",fontSize:12}}>following</span></div></div>
            <div style={{display:"flex",gap:6,marginTop:10,justifyContent:m?"center":"flex-start"}}>
              <button onClick={()=>setEp(true)} style={{padding:"8px 18px",borderRadius:12,...glass,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>Edit Profile</button>
              <button onClick={so} style={{padding:"8px 18px",borderRadius:12,...glass,color:"#fda4af",fontSize:12,fontWeight:700,cursor:"pointer"}}>Sign Out</button></div></div></div>

        <div style={{marginBottom:24}}><div className="sec-title">LISTS</div>
        ...
        {lib.length>0&&<><div className="sec-title">GAMES</div>
          <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(auto-fill,minmax(140px,1fr))",gap:m?8:12}}>{lib.map((g,i)=><GC key={g.id} game={g} delay={i*12} onClick={setSel} mobile={m} ud={ud}/>)}</div></>}
      </div>}
    </main>
