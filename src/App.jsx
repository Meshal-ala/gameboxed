...
const norm = (g) => ({ id: g.id, title: g.name, year: g.released?.slice(0, 4) || "TBA", img: g.background_image || "",
  rating: g.rating ? Math.round(g.rating * 10) / 10 : null, metacritic: g.metacritic, reviews: g.ratings_count || 0,
  ...
  screenshots: g.short_screenshots?.map(s => s.image) || [], slug: g.slug });

/* ── Supabase helpers ── */
const loadCloudLibrary = async (userId) => {
  const { data } = await supabase.from("user_games").select("*").eq("user_id", userId);
  const lib = {};
  (data || []).forEach(r => { lib[r.game_id] = { status: r.status, myRating: r.my_rating, title: r.game_title, img: r.game_img }; });
  return lib;
};
const saveToCloud = async (userId, gameId, fields) => {
  const { data: existing } = await supabase.from("user_games").select("id").eq("user_id", userId).eq("game_id", gameId).single();
  if (existing) {
    await supabase.from("user_games").update({ status: fields.status, my_rating: fields.myRating, game_title: fields.title, game_img: fields.img, updated_at: new Date().toISOString() }).eq("id", existing.id);
  } else {
    await supabase.from("user_games").insert({ user_id: userId, game_id: gameId, status: fields.status, my_rating: fields.myRating, game_title: fields.title, game_img: fields.img });
  }
};
...
const Stars = ({ rating, size = 14, interactive, onRate }) => {
  ...
  return <div style={{ display: "flex", gap: 1 }}>{[1,2,3,4,5].map(s =>
    <span key={s} onMouseEnter={() => interactive && setH(s)} onMouseLeave={() => interactive && setH(0)}
      ...
      filter: s <= (h || rating) ? "drop-shadow(0 0 3px rgba(255,215,0,.4))" : "none" }}>★</span>)}</div>;
};
const Ring = ({ pct, size = 52, stroke = 3.5 }) => {
  ...
  return <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
    ...
    <text x={size/2} y={size/2} textAnchor="middle" dominantBaseline="central"
      ...
      style={{ transform: "rotate(90deg)", transformOrigin: "center" }}>{pct}%</text></svg>;
};
...
const Section = ({ title, action, onAction, children, mobile }) => (
  <div style={{ marginBottom: 32 }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
    <h2 style={{ fontFamily: "'Outfit',sans-serif", fontSize: mobile ? 17 : 19, fontWeight: 800 }}>{title}</h2>
    {action && <span onClick={onAction} style={{ fontSize: 12, color: "#00d4ff", cursor: "pointer", fontWeight: 700 }}>{action}</span>}</div>{children}</div>);
...
const AuthModal = ({ onClose, onAuth }) => {
  ...
  const inputStyle = { width: "100%", padding: "12px 14px", borderRadius: 10, border: "1px solid rgba(255,255,255,.1)",
    background: "rgba(255,255,255,.05)", color: "#fff", fontSize: 14, outline: "none", marginBottom: 10 };

  return <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,.9)", backdropFilter: "blur(24px)",
    display: "flex", alignItems: "center", justifyContent: "center", animation: "fadeIn .2s", padding: 16 }}>
    <div onClick={e => e.stopPropagation()} style={{ background: "#0c0c18", borderRadius: 24, width: "100%", maxWidth: 400,
      padding: 32, border: "1px solid rgba(255,255,255,.06)", animation: "slideUp .35s cubic-bezier(.22,1,.36,1)" }}>

      {sent ? <>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>📧</div>
          <h2 style={{ fontFamily: "'Outfit'", fontSize: 22, fontWeight: 900, marginBottom: 8 }}>Check Your Email</h2>
          <p style={{ color: "#888", fontSize: 14, lineHeight: 1.6 }}>We sent a confirmation link to <span style={{ color: "#00d4ff" }}>{email}</span>. Click it to activate your account.</p>
          <button onClick={onClose} style={{ marginTop: 20, padding: "10px 24px", borderRadius: 10, border: "none",
            background: "linear-gradient(135deg,#00d4ff,#0088bb)", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Got it</button>
        </div>
      </> : <>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: "linear-gradient(135deg,#00d4ff,#8b5cf6)",
            display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 900, margin: "0 auto 12px" }}>G</div>
          <h2 style={{ fontFamily: "'Outfit'", fontSize: 24, fontWeight: 900 }}>{mode === "login" ? "Welcome Back" : "Create Account"}</h2>
          <p style={{ color: "#666", fontSize: 13, marginTop: 4 }}>{mode === "login" ? "Sign in to sync your library" : "Join Gameboxed and start tracking"}</p>
        </div>

        {error && <div style={{ padding: "10px 14px", borderRadius: 10, background: "#ff444415", border: "1px solid #ff444430",
          color: "#ff6666", fontSize: 13, marginBottom: 14 }}>{error}</div>}

        {mode === "signup" && <input placeholder="Your name" value={name} onChange={e => setName(e.target.value)} style={inputStyle} />}
        <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} />
        <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)}
          onKeyDown={e => e.key === "Enter" && handleSubmit()} style={inputStyle} />

        <button onClick={handleSubmit} disabled={loading}
          style={{ width: "100%", padding: "12px", borderRadius: 10, border: "none", marginTop: 4,
            background: loading ? "#333" : "linear-gradient(135deg,#00d4ff,#0088bb)",
        ...
        <div style={{ textAlign: "center", marginTop: 16, fontSize: 13, color: "#666" }}>
          ...
          <span onClick={() => { setMode(mode === "login" ? "signup" : "login"); setError(""); }}
            style={{ color: "#00d4ff", cursor: "pointer", fontWeight: 700 }}>{mode === "login" ? "Sign Up" : "Sign In"}</span>
        </div>
      </>}
    </div>
  </div>;
...
/* ── Game Modal ── */
const Modal = ({ game, onClose, mobile, userData, setUserData, user }) => {
  ...
  const upd = async (f, v) => {
    ...
    const newUd = { ...ud, [f]: v, title: game.title, img: game.img };
    if (f === "myRating") setMr(v); if (f === "status") setSt(v);
    const newData = { ...userData, [game.id]: newUd }; setUserData(newData);
    await saveToCloud(user.id, game.id, newUd);
  };

  return <>
    {showAuth && <AuthModal onClose={() => setShowAuth(false)} onAuth={() => setShowAuth(false)} />}
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,.9)", backdropFilter: "blur(24px)",
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#0c0c18", width: "100%", maxWidth: mobile ? "100%" : 700,
        ...
        <div style={{ position: "relative", height: mobile ? 180 : 220, overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", bottom: 14, left: mobile ? 16 : 24, right: mobile ? 16 : 24, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
            <div><h2 style={{ fontSize: mobile ? 22 : 28, fontWeight: 900, margin: 0 }}>{game.title}</h2>
            ...
            {game.rating && <div style={{ textAlign: "center", flexShrink: 0 }}><div style={{ fontSize: mobile ? 28 : 36, fontWeight: 900, color: "#FFD700", lineHeight: 1 }}>{game.rating}</div>
              <Stars rating={Math.round(game.rating)} size={10} /></div>}</div></div>
        <div style={{ padding: mobile ? "14px 16px 28px" : "18px 24px 28px" }}>
          <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            {game.metacritic && <span style={{ padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700,
              background: (game.metacritic >= 75 ? "#00ff88" : "#FFD700") + "10", color: game.metacritic >= 75 ? "#00ff88" : "#FFD700",
              border: "1px solid " + (game.metacritic >= 75 ? "#00ff88" : "#FFD700") + "28" }}>Metacritic {game.metacritic}</span>}</div>
          {loadingD ? <Loader /> : detail?.description_raw && <p style={{ color: "#777", fontSize: 13, lineHeight: 1.7, marginBottom: 18, maxHeight: 100, overflow: "hidden",
            WebkitMaskImage: "linear-gradient(to bottom, black 60%, transparent)" }}>{detail.description_raw.slice(0, 400)}</p>}

          {/* Rating & Status — requires login */}
          <div style={{ padding: mobile ? 14 : 18, borderRadius: 14, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.05)", marginBottom: 18 }}>
            {!user && <div onClick={() => setShowAuth(true)} style={{ textAlign: "center", padding: "12px 0", cursor: "pointer" }}>
              <span style={{ color: "#00d4ff", fontWeight: 700, fontSize: 13 }}>Sign in to rate & track this game →</span></div>}
            {user && <>
              <div style={{ fontSize: 10, color: "#555", fontWeight: 700, letterSpacing: ".08em", marginBottom: 6 }}>YOUR RATING</div>
              <Stars rating={mr} size={mobile ? 24 : 26} interactive onRate={v => upd("myRating", v)} />
              <div style={{ fontSize: 10, color: "#555", fontWeight: 700, letterSpacing: ".08em", marginTop: 14, marginBottom: 6 }}>ADD TO LIBRARY</div>
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                {Object.entries(STATUS_CFG).map(([k, c]) => <button key={k} onClick={() => upd("status", k)} style={{
                  ...
                  color: st === k ? c.color : "#555" }}>{c.icon} {c.label}</button>)}</div>
            </>}
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (u) loadCloudLibrary(u.id).then(setUserData);
  ...
  useEffect(() => {
    ...
    ]).then(([p, b, n, u]) => { setPopular(p.map(norm)); setBestOfAllTime(b.map(norm)); setNewReleases(n.map(norm)); setUpcoming(u.map(norm)); setLoading(false); });
  }, []);
  ...
  useEffect(() => {
    ...
    setSearching(true);
    searchTimer.current = setTimeout(() => { searchGamesAPI(search).then(r => { setSearchResults(r.map(norm)); setSearching(false); }); }, 400);
  }, [search]);
  ...
  const handleSignOut = async () => { await supabase.auth.signOut(); setUser(null); setUserData({}); };
  const displayName = user?.user_metadata?.display_name || user?.email?.split("@")[0] || "M";
  ...
  return <div style={{ fontFamily: "'DM Sans','Outfit',system-ui,sans-serif", background: "#08080f", color: "#fff", minHeight: "100vh" }}>
    ...
    {!mobile && <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,8,15,.82)", backdropFilter: "blur(24px) saturate(1.4)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ position: "relative" }}><input placeholder="Search 500k+ games..." value={search} onChange={e => { setSearch(e.target.value); if (e.target.value) setPage("search"); }}
          ...
          onFocus={e => { e.target.style.width = "280px"; }} onBlur={e => { e.target.style.width = "200px"; }} />
        ...
        {user ? <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div onClick={handleSignOut} title="Sign Out" style={{ width: 30, height: 30, borderRadius: 7, background: "linear-gradient(135deg,#00d4ff,#8b5cf6)",
            display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>{initial}</div></div>
        : <button onClick={() => setShowAuth(true)} style={{ padding: "6px 14px", borderRadius: 7, border: "none",
            background: "linear-gradient(135deg,#00d4ff,#8b5cf6)", color: "#fff", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>Sign In</button>}
      </div></nav>}
    ...
    {mobile && <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(8,8,15,.9)", backdropFilter: "blur(20px)",
      ...
      {searchOpen ? <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {user ? <div onClick={handleSignOut} style={{ width: 28, height: 28, borderRadius: 7, background: "linear-gradient(135deg,#00d4ff,#8b5cf6)",
            display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>{initial}</div>
          : <button onClick={() => setShowAuth(true)} style={{ padding: "5px 12px", borderRadius: 6, border: "none",
              background: "linear-gradient(135deg,#00d4ff,#8b5cf6)", color: "#fff", fontSize: 10, fontWeight: 800, cursor: "pointer" }}>Sign In</button>}
        </div></>}
    </div>}
    ...
    <main style={{ maxWidth: 1200, margin: "0 auto", padding: mobile ? "12px 14px 100px" : "20px 18px 80px" }}>
      {page === "search" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        </div> : search && <div style={{ textAlign: "center", padding: 60, color: "#444" }}><div style={{ fontSize: 48, marginBottom: 10 }}>🔍</div><div>No games found</div></div>}</div>}
      ...
      {page === "home" && <div style={{ animation: "fadeIn .4s" }}>
        <div style={{ borderRadius: mobile ? 18 : 22, padding: mobile ? "28px 22px" : "44px 36px", marginBottom: mobile ? 24 : 36,
          ...
          <p style={{ color: "#666", fontSize: mobile ? 13 : 14, maxWidth: 460, lineHeight: 1.7, marginBottom: mobile ? 18 : 24, position: "relative" }}>Discover 500,000+ games. Rate, review, and build your collection.</p>
          <div style={{ display: "flex", gap: 10, position: "relative" }}>
            {!user ? <button onClick={() => setShowAuth(true)} style={{ padding: "10px 22px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#00d4ff,#0088bb)", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 20px rgba(0,212,255,.25)" }}>Get Started — Free →</button>
      ...
      {page === "library" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        {!user ? <div style={{ textAlign: "center", padding: 60, color: "#555" }}>
          <div style={{ fontSize: 56, marginBottom: 12 }}>🔒</div><div style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>Sign in to access your library</div>
          <div style={{ fontSize: 13, color: "#444", marginBottom: 20 }}>Your games sync across all your devices</div>
          <button onClick={() => setShowAuth(true)} style={{ padding: "10px 24px", borderRadius: 10, border: "none",
            background: "linear-gradient(135deg,#00d4ff,#0088bb)", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Sign In</button></div>
        : libraryGames.length > 0 ? <>
          ...
          <div style={{ fontSize: 13, color: "#444", marginBottom: 20 }}>Search for games and add them!</div>
      ...
      {page === "explore" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        {loading ? <Loader /> : <>
          <Section title="🏆 Highest Metacritic" mobile={mobile}>
            <div className="hide-scroll" style={{ display: "flex", gap: mobile ? 10 : 14, overflowX: "auto", paddingBottom: 4 }}>
      ...
      {page === "stats" && <div style={{ animation: "fadeIn .4s" }}>
        ...
        {!user ? <div style={{ textAlign: "center", padding: 60, color: "#555" }}>
          <div style={{ fontSize: 48, marginBottom: 10 }}>🔒</div><div style={{ fontSize: 15, fontWeight: 700 }}>Sign in to see your stats</div>
          <button onClick={() => setShowAuth(true)} style={{ marginTop: 16, padding: "10px 24px", borderRadius: 10, border: "none",
            background: "linear-gradient(135deg,#00d4ff,#0088bb)", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Sign In</button></div>
        : libraryGames.length > 0 ? (() => {
          ...
          <div style={{ fontSize: 13, color: "#444", marginTop: 4 }}>Add games to see stats!</div></div>}
      </div>}
    </main>
    ...
    {sel && <Modal game={sel} onClose={() => setSel(null)} mobile={mobile} userData={userData} setUserData={setUserData} user={user} />}
  </div>;
}
