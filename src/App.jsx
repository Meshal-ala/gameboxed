...
const getFollowersList=async u=>{const{data}=await supabase.from("follows").select("follower_id,profiles!follows_follower_id_fkey(id,display_name,username,avatar_url)").eq("following_id",u);return(data||[]).map(d=>d.profiles).filter(Boolean)};
const getFollowingList=async u=>{const{data}=await supabase.from("follows").select("following_id,profiles!follows_following_id_fkey(id,display_name,username,avatar_url)").eq("follower_id",u);return(data||[]).map(d=>d.profiles).filter(Boolean)};
const getAvUrl=url=>url?`${SU}/storage/v1/object/public/avatars/${url}?v=${Date.now()}`:null;
...
const GC=({game:g,onClick,delay=0,mobile:m,ud,sz="md"})=>{const[hov,setHov]=useState(false);const[vis,setVis]=useState(false);const[err,setErr]=useState(false);
  ...
  return<div onClick={()=>onClick?.(g)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    style={{borderRadius:xs?6:10,overflow:"hidden",cursor:"pointer",position:"relative",aspectRatio:xs?"1/1.2":"2/3",
      ...
      outline:hov?"1px solid rgba(16,185,129,.25)":"1px solid #1e293b"}}>
    {!err&&g.img?<img src={g.img} alt={g.title} onError={()=>setErr(true)} loading="lazy"
      ...
      :<div style={{width:"100%",height:"100%",background:"#1e293b",display:"flex",alignItems:"center",justifyContent:"center",fontSize:xs?14:24,color:"#334155"}}>🎮</div>}
    ...
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:xs?"3px 4px":"7px 8px"}}>
      ...
      {!xs&&g.rating&&<div style={{display:"flex",alignItems:"center",gap:2,marginTop:2}}><span style={{fontSize:8,color:"#f59e0b",fontWeight:800}}>★ {g.rating}</span></div>}
...
const Hero=({games,onClick,m})=>{const[idx,setIdx]=useState(0);const g=games[idx];
  ...
  if(!g)return null;
  return<div onClick={()=>onClick?.(g)} style={{position:"relative",width:"100%",aspectRatio:m?"16/10":"21/9",borderRadius:m?12:16,overflow:"hidden",cursor:"pointer",marginBottom:m?16:24,border:"1px solid #1e293b"}}>
    ...
    <div style={{position:"absolute",bottom:m?16:28,left:m?16:28,right:m?80:null}}>
      ...
      <div style={{fontSize:m?11:13,color:"#94a3b8",marginTop:4}}>{g.year} · {g.genre}</div>
    </div>
    <div style={{position:"absolute",bottom:m?16:28,right:m?16:28,display:"flex",gap:4}}>
      {games.slice(0,5).map((_,i)=><div key={i} onClick={e=>{e.stopPropagation();setIdx(i)}} style={{width:i===idx?16:6,height:6,borderRadius:3,background:i===idx?"#10b981":"#475569",transition:"all .3s",cursor:"pointer"}}/>)}</div>
  </div>};
...
/* Edit Profile - FIXED persistence */
const EP=({profile:p,onClose,onSave,userId})=>{const[n,setN]=useState(p?.display_name||"");const[u,setU]=useState(p?.username||"");const[b,setB]=useState(p?.bio||"");const[ld,setLd]=useState(false);const[upl,setUpl]=useState(false);const fr=useRef();
  ...
  const hf=async e=>{const f=e.target.files[0];if(!f)return;setUpl(true);try{await upAv(userId,f);window.location.reload()}catch(e){console.error(e);setUpl(false)}};
...
const FollowListModal=({userId,type,onClose,mobile:m,setVU})=>{const[list,setList]=useState([]);const[ld,setLd]=useState(true);
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:1800,background:"#0f172af0",backdropFilter:"blur(16px)",display:"flex",alignItems:m?"flex-end":"center",justifyContent:"center",animation:"fadeIn .15s",padding:m?0:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#1e293b",width:"100%",maxWidth:m?"100%":400,maxHeight:m?"80vh":"70vh",borderRadius:m?"20px 20px 0 0":20,overflow:"auto",border:m?"none":"1px solid #334155",animation:m?"slideFromBottom .25s ease":"slideUp .25s ease"}}>
      ...
      <div style={{padding:"20px 20px 24px"}}>
        ...
        {ld?<Loader/>:list.length>0?list.map(p=>
          <div key={p.id} onClick={()=>{onClose();setVU(p.id)}} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",borderRadius:10,marginBottom:4,cursor:"pointer",transition:"background .15s",background:"transparent"}}
            ...
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#f1f5f9"}}>{p.display_name||"User"}</div>
              {p.username&&<div style={{fontSize:11,color:"#64748b"}}>@{p.username}</div>}</div></div>)
        :<div style={{textAlign:"center",padding:30,color:"#64748b",fontSize:13}}>No {type} yet</div>}
      </div></div></div>};

/* User Profile */
const UP=({viewId,onClose,me,mobile:m,onFollow,setVU})=>{const[p,setP]=useState(null);const[fc,setFc]=useState({followers:0,following:0});const[gs,setGs]=useState([]);const[isF,setIsF]=useState(false);const[ld,setLd]=useState(true);const[flModal,setFlModal]=useState(null);
  ...
  return<><div onClick={onClose} style={{position:"fixed",inset:0,zIndex:1500,background:"#0f172af0",backdropFilter:"blur(16px)",display:"flex",alignItems:m?"flex-end":"center",justifyContent:"center",animation:"fadeIn .15s",padding:m?0:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#1e293b",width:"100%",maxWidth:m?"100%":440,maxHeight:m?"88vh":"auto",borderRadius:m?"20px 20px 0 0":20,overflow:"auto",border:m?"none":"1px solid #334155"}}>
      {m&&<div onClick={onClose} style={{display:"flex",justifyContent:"center",padding:"10px 0 0"}}><div style={{width:32,height:3,borderRadius:2,background:"#334155"}}/></div>}
      <div style={{padding:"28px 24px",textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:20,justifyContent:"center",marginTop:14}}>
          ...
          <div onClick={()=>setFlModal("followers")} style={{cursor:"pointer"}}><div style={{fontSize:16,fontWeight:900,color:"#f1f5f9"}}>{fc.followers}</div><div style={{fontSize:9,color:"#10b981"}}>Followers</div></div>
          <div onClick={()=>setFlModal("following")} style={{cursor:"pointer"}}><div style={{fontSize:16,fontWeight:900,color:"#f1f5f9"}}>{fc.following}</div><div style={{fontSize:9,color:"#10b981"}}>Following</div></div>
        </div>
    ...
    {flModal&&<FollowListModal userId={viewId} type={flModal} onClose={()=>setFlModal(null)} mobile={m} setVU={v=>{setFlModal(null);onClose();setVU(v)}}/>}</>};
    ...
    <div onClick={e=>e.stopPropagation()} style={{background:"#1e293b",width:"100%",maxWidth:m?"100%":660,maxHeight:m?"93vh":"88vh",borderRadius:m?"20px 20px 0 0":20,overflow:"auto",border:m?"none":"1px solid #334155"}}>
      ...
      <div style={{position:"relative",height:m?170:220,overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",bottom:14,left:m?16:24,right:m?16:24}}>
          <h2 style={{fontFamily:"'Outfit'",fontSize:m?22:28,fontWeight:900,margin:0,color:"#f1f5f9"}}>{g.title}</h2>
      ...
      <div style={{padding:m?"12px 16px 32px":"18px 24px 28px"}}>
        <div style={{display:"flex",gap:0,marginBottom:16,background:"#0f172a",borderRadius:12,overflow:"hidden",border:"1px solid #1e293b"}}>
          {[{l:"Community",v:g.rating,c:"#f59e0b"},{l:"Metacritic",v:g.mc,c:g.mc>=75?"#10b981":"#f59e0b"},{l:"Yours",v:mr||"—",c:"#3b82f6"}].map((s,i)=>
            <div key={i} style={{flex:1,padding:"12px 8px",textAlign:"center",borderRight:i<2?"1px solid #1e293b":"none"}}>
              ...
              <div style={{fontSize:20,fontWeight:900,color:typeof s.v==="number"?s.c:"#475569",marginTop:2}}>{s.v||"—"}</div></div>)}</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
          <div><div style={{fontSize:9,color:"#64748b",fontWeight:700,marginBottom:6,letterSpacing:".08em"}}>YOUR RATING</div><Stars rating={mr} size={m?22:26} interactive onRate={v=>sv("myRating",v)}/></div>
          <div><div style={{fontSize:9,color:"#64748b",fontWeight:700,marginBottom:6,letterSpacing:".08em"}}>STATUS</div>
        ...
        <div style={{display:"flex",gap:4,marginBottom:16}}>{g.pf.map((p,i)=><span key={i} style={{fontSize:9,fontWeight:700,padding:"4px 8px",borderRadius:6,background:"#0f172a",color:p.c,border:"1px solid #1e293b"}}>{p.n}</span>)}</div>
        <div style={{display:"flex",borderBottom:"1px solid #334155",marginBottom:14}}>
          {["about","reviews","media"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"9px 14px",background:"none",border:"none",fontSize:12,fontWeight:700,cursor:"pointer",color:tab===t?"#10b981":"#475569",borderBottom:tab===t?"2px solid #10b981":"2px solid transparent",textTransform:"capitalize"}}>{t}{t==="reviews"&&rvs.length>0?` ${rvs.length}`:""}</button>)}</div>
        ...
        {tab==="reviews"&&<div>
          {me&&<div style={{marginBottom:16,padding:14,borderRadius:12,background:"#0f172a",border:"1px solid #1e293b"}}>
            ...
            <textarea value={rt} onChange={e=>setRt(e.target.value)} rows={3} placeholder="Share your thoughts..." style={{width:"100%",marginTop:8,padding:"10px 12px",borderRadius:8,border:"1px solid #334155",background:"#1e293b",color:"#f1f5f9",fontSize:13,outline:"none",resize:"none",fontFamily:"inherit"}}/>
          ...
          {rvs.map(r=><div key={r.id} style={{padding:12,borderRadius:10,background:"#0f172a",border:"1px solid #1e293b",marginBottom:6}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              ...
              <div style={{flex:1}}><span style={{fontSize:12,fontWeight:700,color:"#f1f5f9",cursor:"pointer"}} onClick={()=>{onClose();setVU(r.user_id)}}>{r.profiles?.display_name||"User"}</span></div>
          ...
          {!rvs.length&&<div style={{textAlign:"center",padding:24,color:"#475569",fontSize:13}}>No reviews yet</div>}</div>}
...
export default function App(){
  ...
  const[sa,setSa]=useState(false);const[ep,setEp]=useState(false);const[vU,setVU]=useState(null);const[flModal,setFlModal]=useState(null);
  ...
  const[myL,setMyL]=useState([]);const[nLN,setNLN]=useState("");const[feed,setFeed]=useState([]);const[recentRev,setRecentRev]=useState([]);
  const stt=useRef(null);
  const rf=()=>{if(user)loadFeed(user.id).then(setFeed);else loadAllFeed().then(setFeed);loadRR().then(setRecentRev)};

  useEffect(()=>{supabase.auth.getSession().then(({data:{session}})=>{const u=session?.user||null;setUser(u);if(u){lcl(u.id).then(setUd);lp(u.id).then(setProf);loadLists(u.id).then(setMyL);loadFeed(u.id).then(setFeed);getFC(u.id).then(setFc)}else loadAllFeed().then(setFeed)});loadRR().then(setRecentRev);
  ...
  return<div style={{fontFamily:"'DM Sans','Outfit',system-ui,sans-serif",background:"#0f172a",color:"#e2e8f0",minHeight:"100vh"}}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,100..1000&family=Outfit:wght@100..900&display=swap');
      *{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#334155;border-radius:2px}
      ...
      body{background:#0f172a;overflow-x:hidden}img{-webkit-user-drag:none}.hs::-webkit-scrollbar{display:none}.hs{-ms-overflow-style:none;scrollbar-width:none}
      @media(max-width:767px){*{-webkit-tap-highlight-color:transparent}}input::placeholder,textarea::placeholder{color:#475569}`}</style>
    ...
    {flModal&&<FollowListModal userId={user?.id} type={flModal} onClose={()=>setFlModal(null)} mobile={m} setVU={setVU}/>}

    {/* Desktop Nav */}
    {!m&&<nav style={{position:"sticky",top:0,zIndex:100,background:"#0f172ae8",backdropFilter:"blur(16px)",borderBottom:"1px solid #1e293b",padding:"0 28px",height:48,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",alignItems:"center",gap:24}}>
        ...
        <div style={{display:"flex",gap:1}}>{NAV.filter(n=>n.id!=="profile").map(n=><button key={n.id} onClick={()=>{setPg(n.id);setQ("")}} style={{padding:"5px 12px",borderRadius:6,border:"none",background:pg===n.id?"#1e293b":"transparent",color:pg===n.id?"#e2e8f0":"#64748b",cursor:"pointer",fontSize:12,fontWeight:600}}>{n.l}</button>)}</div></div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{position:"relative"}}><input placeholder="Search..." value={q} onChange={e=>{setQ(e.target.value);if(e.target.value)setPg("search")}}
          style={{padding:"7px 12px 7px 30px",borderRadius:8,border:"1px solid #334155",background:"#1e293b",color:"#f1f5f9",fontSize:12,width:180,outline:"none"}}/>
    ...
    {m&&<div style={{position:"sticky",top:0,zIndex:100,background:"#0f172ae8",backdropFilter:"blur(16px)",padding:"0 12px",height:44,display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #1e293b"}}>
      {qO?<div style={{flex:1,display:"flex",alignItems:"center",gap:6}}>
        <input autoFocus placeholder="Search..." value={q} onChange={e=>{setQ(e.target.value);if(e.target.value)setPg("search")}}
          style={{flex:1,padding:"7px 12px",borderRadius:8,border:"1px solid #10b98133",background:"#1e293b",color:"#f1f5f9",fontSize:13,outline:"none"}}/>
    ...
    <main style={{maxWidth:1100,margin:"0 auto",padding:m?"6px 10px 82px":"12px 24px 40px"}}>
      ...
      {pg==="search"&&<div style={{animation:"fadeIn .2s"}}>
        ...
        {sng?<Loader/>:<>
          ...
          {sT==="people"&&(pSr.length>0?<div style={{display:"flex",flexDirection:"column",gap:6}}>{pSr.map(p=>
            <div key={p.id} onClick={()=>setVU(p.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:12,background:"#1e293b",border:"1px solid #334155",cursor:"pointer"}}
              onMouseEnter={e=>e.currentTarget.style.borderColor="#10b98133"} onMouseLeave={e=>e.currentTarget.style.borderColor="#334155"}>
              <Av url={p.avatar_url} name={p.display_name} size={40}/>
              <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#f1f5f9"}}>{p.display_name||"User"}</div>{p.username&&<div style={{fontSize:11,color:"#64748b"}}>@{p.username}</div>}</div>
              <span style={{fontSize:11,color:"#475569"}}>→</span></div>)}</div>:q&&<p style={{textAlign:"center",padding:40,color:"#475569"}}>No people</p>)}
          {sT==="lists"&&(lSr.length>0?<div style={{display:"flex",flexDirection:"column",gap:6}}>{lSr.map(l=><div key={l.id} style={{padding:"12px 14px",borderRadius:12,background:"#1e293b",border:"1px solid #334155"}}>
      ...
      {pg==="home"&&<div style={{animation:"fadeIn .3s"}}>
        {!user&&!ld&&<div style={{textAlign:"center",padding:m?"16px 0 20px":"24px 0 28px",borderBottom:"1px solid #1e293b",marginBottom:m?12:20}}>
        ...
        {ld?<Loader/>:<div style={{display:m?"block":"grid",gridTemplateColumns:"1fr 280px",gap:20}}>
          ...
          {!m&&<div>
            {recentRev.length>0&&<div style={{marginBottom:20}}><h3 style={{fontSize:11,fontWeight:800,color:"#64748b",letterSpacing:".1em",marginBottom:10}}>RECENT REVIEWS</h3>
              {recentRev.slice(0,5).map(r=><div key={r.id} style={{padding:"10px 12px",borderRadius:10,background:"#1e293b",border:"1px solid #334155",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                  ...
                  <span style={{fontSize:9,color:"#334155"}}>{tA(r.created_at)}</span></div>
            ...
            <div><h3 style={{fontSize:11,fontWeight:800,color:"#64748b",letterSpacing:".1em",marginBottom:10}}>ACTIVITY</h3>
              {feed.slice(0,8).map(a=><div key={a.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:"1px solid #1e293b",fontSize:11}}>
      ...
      {pg==="feed"&&user&&<div style={{animation:"fadeIn .2s"}}>
        ...
        {feed.length>0?feed.map(a=><div key={a.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,background:"#1e293b",border:"1px solid #334155",marginBottom:4}}>
      ...
      {pg==="library"&&user&&<div style={{animation:"fadeIn .2s"}}>
        ...
        {lib.length>0?<><div className="hs" style={{display:"flex",gap:4,marginBottom:12,overflowX:"auto"}}>
          {[["all","All"],["playing","Playing"],["completed","Completed"],["backlog","Backlog"],["wishlist","Wishlist"],["dropped","Dropped"]].map(([k,l])=>
            <button key={k} onClick={()=>setLf(k)} style={{padding:"5px 12px",borderRadius:8,fontSize:11,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",border:"1px solid "+(lf===k?"#10b981":"#334155"),background:lf===k?"#10b98115":"#1e293b",color:lf===k?"#10b981":"#64748b"}}>{l} {k==="all"?lib.length:lib.filter(g=>ud[g.id]?.status===k).length}</button>)}</div>
        ...
        {lib.length>0?(()=>{const bs=s=>lib.filter(g=>ud[g.id]?.status===s).length;const rt=lib.filter(g=>ud[g.id]?.myRating);const av=rt.length?(rt.reduce((s,g)=>s+ud[g.id].myRating,0)/rt.length).toFixed(1):"—";
          return<><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:m?6:10,marginBottom:20}}>
            {[{l:"Games",v:lib.length,c:"#3b82f6"},{l:"Done",v:bs("completed"),c:"#10b981"},{l:"Playing",v:bs("playing"),c:"#8b5cf6"},{l:"Avg ★",v:av,c:"#f59e0b"}].map((s,i)=>
              <div key={i} style={{padding:m?10:14,borderRadius:10,textAlign:"center",background:"#1e293b",border:"1px solid #334155"}}>
        ...
        <div style={{display:"flex",alignItems:m?"center":"flex-start",gap:m?14:20,marginBottom:24,flexDirection:m?"column":"row"}}>
          ...
          <div style={{textAlign:m?"center":"left",flex:1}}>
            ...
            <div style={{display:"flex",gap:6,marginTop:10,justifyContent:m?"center":"flex-start"}}>
              <button onClick={()=>setEp(true)} style={{padding:"7px 16px",borderRadius:8,border:"1px solid #334155",background:"transparent",color:"#e2e8f0",fontSize:11,fontWeight:700,cursor:"pointer"}}>Edit Profile</button>
              <button onClick={so} style={{padding:"7px 16px",borderRadius:8,border:"1px solid #334155",background:"transparent",color:"#ef4444",fontSize:11,fontWeight:700,cursor:"pointer"}}>Sign Out</button></div></div></div>

        <div style={{marginBottom:20}}>
          <h3 style={{fontSize:11,fontWeight:800,color:"#64748b",letterSpacing:".1em",marginBottom:10}}>LISTS</h3>
          {myL.map(l=><div key={l.id} style={{padding:"10px 12px",borderRadius:8,background:"#1e293b",border:"1px solid #334155",marginBottom:4,fontSize:13,fontWeight:700,color:"#f1f5f9"}}>📝 {l.title}</div>)}
          <div style={{display:"flex",gap:6,marginTop:6}}><input placeholder="New list..." value={nLN} onChange={e=>setNLN(e.target.value)} onKeyDown={e=>e.key==="Enter"&&hcl()}
            style={{flex:1,padding:"9px 12px",borderRadius:8,border:"1px solid #334155",background:"#1e293b",color:"#f1f5f9",fontSize:12,outline:"none"}}/>
    ...
    {m&&<div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:90,background:"#0f172af5",backdropFilter:"blur(16px)",borderTop:"1px solid #1e293b",display:"flex",paddingTop:4,paddingBottom:"max(env(safe-area-inset-bottom,10px),10px)"}}>
