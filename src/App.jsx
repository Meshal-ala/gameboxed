...
const PP = { 1:{name:"PC",icon:"🖥️",color:"#00d4ff"},2:{name:"PlayStation",icon:"🎮",color:"#006FCD"},3:{name:"Xbox",icon:"🟢",color:"#107C10"},7:{name:"Nintendo",icon:"🔴",color:"#E60012"} };
const SC = { completed:{color:"#00ff88",label:"Completed",icon:"✓"},playing:{color:"#00d4ff",label:"Playing",icon:"▶"},wishlist:{color:"#FFD700",label:"Wishlist",icon:"☆"},dropped:{color:"#ff4444",label:"Dropped",icon:"✕"},backlog:{color:"#8b5cf6",label:"Backlog",icon:"◷"} };

const fg=async(p="")=>{try{const r=await fetch(`${API}/games?key=${API_KEY}&page_size=12${p}`);const d=await r.json();return d.results||[]}catch{return[]}};
const fgd=async(id)=>{try{const r=await fetch(`${API}/games/${id}?key=${API_KEY}`);return await r.json()}catch{return null}};
const sga=async(q)=>{try{const r=await fetch(`${API}/games?key=${API_KEY}&search=${encodeURIComponent(q)}&page_size=20&search_precise=true`);const d=await r.json();return d.results||[]}catch{return[]}};
...
const lcl=async(uid)=>{const{data}=await supabase.from("user_games").select("*").eq("user_id",uid);const lib={};(data||[]).forEach(r=>{lib[r.game_id]={status:r.status,myRating:r.my_rating,title:r.game_title,img:r.game_img}});return lib};
const stc=async(uid,gid,f)=>{const{data:ex}=await supabase.from("user_games").select("id").eq("user_id",uid).eq("game_id",gid).single();if(ex){await supabase.from("user_games").update({status:f.status,my_rating:f.myRating,game_title:f.title,game_img:f.img,updated_at:new Date().toISOString()}).eq("id",ex.id)}else{await supabase.from("user_games").insert({user_id:uid,game_id:gid,status:f.status,my_rating:f.myRating,game_title:f.title,game_img:f.img})}};
const lp=async(uid)=>{const{data}=await supabase.from("profiles").select("*").eq("id",uid).single();return data};
const up=async(uid,f)=>{await supabase.from("profiles").update(f).eq("id",uid)};
...
const GC=({game:g,onClick,delay=0,mobile:m,userData:ud,size="normal"})=>{
  ...
  const u=ud?.[g.id];const sm=size==="small";
  return<div onClick={()=>onClick?.(g)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    style={{borderRadius:sm?8:10,overflow:"hidden",cursor:"pointer",position:"relative",aspectRatio:"2/3",
    ...
    {!err&&g.img?<img src={g.img} alt={g.title} onError={()=>setErr(true)} loading="lazy"
      ...
      :<div style={{width:"100%",height:"100%",background:"linear-gradient(135deg,#16213e,#0f3460)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:sm?24:32,color:"#ffffff15"}}>🎮</div>}
    <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,.88) 0%,rgba(0,0,0,.05) 55%,transparent 100%)"}}/>
    {u?.status&&<div style={{position:"absolute",top:sm?4:6,left:sm?4:6,padding:sm?"1px 5px":"2px 7px",borderRadius:10,background:"rgba(0,0,0,.6)",fontSize:sm?7:8,fontWeight:800,color:SC[u.status]?.color,letterSpacing:".04em"}}>{SC[u.status]?.label.toUpperCase()}</div>}
    {g.metacritic&&!sm&&<div style={{position:"absolute",top:6,right:6,padding:"2px 5px",borderRadius:4,background:"rgba(0,0,0,.6)",fontSize:9,fontWeight:900,color:g.metacritic>=75?"#00ff88":g.metacritic>=50?"#FFD700":"#ff4444"}}>{g.metacritic}</div>}
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:sm?"6px":"8px 9px"}}>
      <div style={{fontSize:sm?10:12,fontWeight:700,lineHeight:1.2,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{g.title}</div>
      {!sm&&g.rating&&<div style={{display:"flex",alignItems:"center",gap:2,marginTop:2}}><Stars rating={Math.round(g.rating)} size={7}/><span style={{fontSize:8,color:"#FFD700",fontWeight:700}}>{g.rating}</span></div>}
      {!sm&&<div style={{fontSize:8,color:"rgba(255,255,255,.35)",marginTop:2}}>{g.year}</div>}
    </div></div>};

/* ── Wide Featured Card ── */
const WC=({game:g,onClick,mobile:m,userData:ud,label})=>{
  const[hov,setHov]=useState(false);const u=ud?.[g.id];
  return<div onClick={()=>onClick?.(g)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    style={{borderRadius:12,overflow:"hidden",cursor:"pointer",position:"relative",aspectRatio:m?"16/9":"21/9",
      boxShadow:hov&&!m?"0 16px 40px rgba(0,0,0,.5)":"0 2px 12px rgba(0,0,0,.2)",transition:"all .3s"}}>
    ...
    {u?.status&&<div style={{position:"absolute",top:8,right:8,padding:"2px 8px",borderRadius:10,background:"rgba(0,0,0,.6)",fontSize:8,fontWeight:800,color:SC[u.status]?.color}}>{SC[u.status]?.label.toUpperCase()}</div>}
    <div style={{position:"absolute",bottom:m?14:20,left:m?14:24}}>
      {label&&<div style={{fontSize:10,color:"#00d4ff",fontWeight:700,letterSpacing:".08em",marginBottom:4}}>{label}</div>}
      <h3 style={{fontSize:m?18:28,fontWeight:900,margin:0,lineHeight:1.15}}>{g.title}</h3>
      <div style={{fontSize:m?10:12,color:"rgba(255,255,255,.4)",marginTop:3}}>{g.year} · {g.genre}{g.metacritic&&" · Metacritic "+g.metacritic}</div>
      {g.rating&&<div style={{display:"flex",alignItems:"center",gap:4,marginTop:4}}><Stars rating={Math.round(g.rating)} size={m?9:11}/><span style={{fontSize:m?10:12,fontWeight:800,color:"#FFD700"}}>{g.rating}</span></div>}
    </div></div>};

/* ── Section ── */
const Sec=({title,children,action,onAction})=>(
  <div style={{marginBottom:28}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <h3 style={{fontSize:13,fontWeight:700,color:"#888",letterSpacing:".04em"}}>{title}</h3>
      {action&&<span onClick={onAction} style={{fontSize:11,color:"#444",cursor:"pointer",fontWeight:600}}>More →</span>}</div>
    {children}</div>);
...
const Auth=({onClose,onAuth})=>{const[mode,setMode]=useState("login");const[email,setEmail]=useState("");const[pw,setPw]=useState("");const[name,setName]=useState("");const[err,setErr]=useState("");const[ld,setLd]=useState(false);const[sent,setSent]=useState(false);
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,.85)",backdropFilter:"blur(24px)",display:"flex",alignItems:"center",justifyContent:"center",animation:"fadeIn .2s",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#111118",borderRadius:20,width:"100%",maxWidth:380,padding:"32px 28px",border:"1px solid rgba(255,255,255,.06)",animation:"slideUp .3s cubic-bezier(.22,1,.36,1)"}}>
      {sent?<div style={{textAlign:"center"}}><div style={{fontSize:48,marginBottom:12}}>📧</div><h2 style={{fontSize:20,fontWeight:800,marginBottom:8}}>Check Your Email</h2><p style={{color:"#888",fontSize:13}}>We sent a link to <span style={{color:"#00d4ff"}}>{email}</span></p>
      ...
      :<><div style={{textAlign:"center",marginBottom:24}}><h2 style={{fontSize:22,fontWeight:900}}>{mode==="login"?"Sign in":"Join Gameboxed"}</h2><p style={{color:"#555",fontSize:12,marginTop:4}}>{mode==="login"?"Welcome back":"Track your gaming journey"}</p></div>
        ...
        <div style={{textAlign:"center",marginTop:14,fontSize:12,color:"#555"}}>{mode==="login"?"New here? ":"Have an account? "}<span onClick={()=>{setMode(mode==="login"?"signup":"login");setErr("")}} style={{color:"#00d4ff",cursor:"pointer",fontWeight:700}}>{mode==="login"?"Create account":"Sign in"}</span></div></>}
    </div></div>};
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,.85)",backdropFilter:"blur(24px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#111118",borderRadius:20,width:"100%",maxWidth:380,padding:"28px 24px",border:"1px solid rgba(255,255,255,.06)"}}>
      ...
      <div style={{display:"flex",gap:8,marginTop:4}}>
        ...
        <button onClick={s} disabled={ld} style={{flex:1,padding:"11px",borderRadius:10,border:"none",background:"#00d4ff",color:"#000",fontSize:13,fontWeight:800,cursor:"pointer"}}>{ld?"...":"Save"}</button></div>
    </div></div>};
    ...
    <div onClick={e=>e.stopPropagation()} style={{background:"#111118",width:"100%",maxWidth:m?"100%":640,maxHeight:m?"92vh":"85vh",borderRadius:m?"20px 20px 0 0":20,overflow:"auto",border:m?"none":"1px solid rgba(255,255,255,.06)",animation:m?"slideFromBottom .3s cubic-bezier(.22,1,.36,1)":"slideUp .3s cubic-bezier(.22,1,.36,1)"}}>
      ...
      <div style={{padding:m?"12px 16px 28px":"16px 20px 24px"}}>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:16,padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
          {g.rating&&<div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#555",fontWeight:700}}>RATING</div><div style={{fontSize:20,fontWeight:900,color:"#FFD700"}}>{g.rating}</div></div>}
          {g.metacritic&&<div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#555",fontWeight:700}}>META</div><div style={{fontSize:20,fontWeight:900,color:g.metacritic>=75?"#00ff88":"#FFD700"}}>{g.metacritic}</div></div>}
          <div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#555",fontWeight:700}}>YOURS</div><div style={{fontSize:20,fontWeight:900,color:"#00d4ff"}}>{mr||"—"}</div></div>
          <div style={{marginLeft:"auto",display:"flex",gap:3}}>{g.platforms.map((p,i)=><span key={i} style={{fontSize:8,fontWeight:700,padding:"3px 6px",borderRadius:4,background:p.color+"10",color:p.color}}>{p.name}</span>)}</div></div>
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          <div style={{flex:1}}><div style={{fontSize:10,color:"#444",fontWeight:700,marginBottom:6,letterSpacing:".06em"}}>RATE</div><Stars rating={mr} size={m?22:24} interactive onRate={v=>sv("myRating",v)}/></div>
          <div style={{flex:1}}><div style={{fontSize:10,color:"#444",fontWeight:700,marginBottom:6,letterSpacing:".06em"}}>STATUS</div>
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{Object.entries(SC).map(([k,c])=><button key={k} onClick={()=>sv("status",k)} style={{padding:"4px 10px",borderRadius:14,fontSize:10,fontWeight:700,cursor:"pointer",border:st===k?"1px solid "+c.color:"1px solid #1a1a2a",background:st===k?c.color+"12":"transparent",color:st===k?c.color:"#555"}}>{c.label}</button>)}</div></div></div>
...
export default function App(){
  ...
  const[sr,setSr]=useState([]);const[ld,setLd]=useState(true);const[sng,setSng]=useState(false);const[lf,setLf]=useState("all");
  ...
  useEffect(()=>{supabase.auth.getSession().then(({data:{session}})=>{const u=session?.user||null;setUser(u);if(u){lcl(u.id).then(setUd);lp(u.id).then(setProf)}});
    const{data:{subscription}}=supabase.auth.onAuthStateChange((_,session)=>{const u=session?.user||null;setUser(u);if(u){lcl(u.id).then(setUd);lp(u.id).then(setProf)}else{setUd({});setProf(null)}});return()=>subscription.unsubscribe()},[]);
  ...
  useEffect(()=>{setLd(true);const td=new Date().toISOString().slice(0,10);const ly=new Date(Date.now()-365*864e5).toISOString().slice(0,10);const ny=new Date(Date.now()+365*864e5).toISOString().slice(0,10);
    Promise.all([
      fg(`&dates=${ly},${td}&ordering=-rating&metacritic=70,100`),
      fg(`&ordering=-metacritic&metacritic=85,100`),
      fg(`&dates=${ly},${td}&ordering=-released`),
      fg(`&dates=${td},${ny}&ordering=-added`),
      fg(`&genres=action&ordering=-rating&metacritic=75,100&page_size=8`),
      fg(`&genres=role-playing-games-rpg&ordering=-rating&metacritic=75,100&page_size=8`),
  ...
  useEffect(()=>{if(st.current)clearTimeout(st.current);if(!q.trim()){setSr([]);return}setSng(true);st.current=setTimeout(()=>{sga(q).then(r=>{setSr(r.map(nm));setSng(false)})},400)},[q]);
  ...
  const sP=async(f)=>{await up(user.id,f);setProf({...prof,...f})};
  ...
  return<div style={{fontFamily:"'DM Sans','Outfit',system-ui,sans-serif",background:"#0b0b10",color:"#fff",minHeight:"100vh"}}>
    ...
    {!m&&<nav style={{position:"sticky",top:0,zIndex:100,background:"rgba(11,11,16,.85)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,.04)",padding:"0 32px",height:50,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{position:"relative"}}><input placeholder="Search games..." value={q} onChange={e=>{setQ(e.target.value);if(e.target.value)setPg("search")}}
    ...
    <main style={{maxWidth:1100,margin:"0 auto",padding:m?"8px 12px 86px":"16px 24px 50px"}}>
      ...
      {pg==="search"&&<div style={{animation:"fadeIn .3s",paddingTop:10}}>
        <h2 style={{fontFamily:"'Outfit'",fontSize:m?18:22,fontWeight:900,marginBottom:14}}>{q?`"${q}"`:"Search"}</h2>
        {sng?<Loader/>:sr.length>0?<div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(auto-fill,minmax(140px,1fr))",gap:m?6:10}}>
          {sr.map((g,i)=><GC key={g.id} game={g} delay={i*25} onClick={setSel} mobile={m} userData={ud}/>)}</div>
        :q&&<div style={{textAlign:"center",padding:50,color:"#444"}}>No results</div>}</div>}
      ...
      {pg==="home"&&<div style={{animation:"fadeIn .4s"}}>
        ...
        {ld?<Loader/>:<>
          {/* Featured wide card */}
          {popular[0]&&<div style={{marginBottom:m?20:28}}><WC game={popular[0]} onClick={setSel} mobile={m} userData={ud} label="TRENDING NOW"/></div>}
          ...
          <Sec title="POPULAR THIS WEEK" action="More →" onAction={()=>setPg("explore")}>
            <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>
              {popular.slice(1,m?7:7).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>

          {/* New Releases */}
          <Sec title="JUST RELEASED">
            <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>
              {fresh.slice(0,m?6:6).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>

          {/* Featured #2 */}
          {best[0]&&<div style={{marginBottom:m?20:28}}><WC game={best[0]} onClick={setSel} mobile={m} userData={ud} label="HIGHEST RATED"/></div>}

          {/* Best of All Time */}
          <Sec title="ALL-TIME GREATS">
            <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>
              {best.slice(1,m?7:7).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>

          {/* Coming Soon */}
          {soon.length>0&&<Sec title="COMING SOON">
            <div className="hs" style={{display:"flex",gap:m?6:10,overflowX:"auto",paddingBottom:4}}>
              {soon.slice(0,10).map((g,i)=><div key={g.id} style={{minWidth:m?100:140,flex:"0 0 auto"}}><GC game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud} size="small"/></div>)}</div></Sec>}

          {/* Top Action */}
          <Sec title="TOP ACTION GAMES">
            <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>
              {action.slice(0,m?6:6).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>

          {/* Top RPG */}
          <Sec title="TOP RPGs">
            <div style={{display:"grid",gridTemplateColumns:m?"repeat(3,1fr)":"repeat(6,1fr)",gap:m?6:10}}>
              {rpg.slice(0,m?6:6).map((g,i)=><GC key={g.id} game={g} delay={i*30} onClick={setSel} mobile={m} userData={ud}/>)}</div></Sec>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {Object.entries(SC).map(([k,c])=>{const cn=bs(k),mx=lib.length||1;
                return<div key={k} style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{width:66,fontSize:11,color:"#666",fontWeight:600,textAlign:"right"}}>{c.label}</span>
                  <div style={{flex:1,height:18,background:"rgba(255,255,255,.03)",borderRadius:4,overflow:"hidden"}}>
                    <div style={{height:"100%",width:cn>0?(cn/mx*100)+"%":"0%",background:c.color,borderRadius:4,opacity:.7,transition:"width .8s",display:"flex",alignItems:"center",paddingLeft:5}}>
